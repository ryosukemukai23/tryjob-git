...
class articleLogic
{
	...
	static function getSearchPageChange( &$gm, $loginUserType, $loginUserRank, $row, $pagejumpNum, $resultNum, $phpName, $param )
	{
	 ...
	 * 公開条件の取得
	 */
	static function getOpenConditionTable($table=null)
	{
	 ...
	 * 公開アイテムか確認
	 */
	static function checkOpen( $db, $rec )
	{
		global $ACTIVE_ACCEPT;
	 ...
	 * 作成者の退会フラグを変更する
	 */
	static function changeOwnerWithdrawal($owner, $val=null)
	{
	 ...
	 * 詳細ページのメタタグの設定
	 */
	static function setRobots($gm, $max=0, $row=0)
	{
		...
		$uri = ( 'on' == $_SERVER[ 'HTTPS' ] ? 'https://' : 'https://' ) . $_SERVER[ 'HTTP_HOST' ] . $uri;
	 ...
	 * @return 配列
	 */
	static function getDataList( $table )
	{
	 ...
	 */
	static function formatInfoData( &$rec )
	{
		global $FileBase;
	...
	static function setSearchWord( $iId, $iPartsRec ){
		$db = GMList::getDB(self::$type);
	 ...
	 * @param $iPrec 新しいレコード
	 */
	static function SearchWord($iSearchWord, $iPrec ) {
	...
	static function saveDataScheme($url,$directory)
	{
	...
	static function changeWebp($url_img)
	{
		$tmp = $url_img;
	...
	static function changeJapaneseDomain($url)
	{
		global $IDNA2;
	...
	static function checkUrlImg($url_img)
	{
	...
	static function sslConTextSetting($url,$ignore_erros = true)
	{
		global $caFile;
		global $caFileUse;
	 ...
	 */
	static function setRelationSearchTable( $aTable, $aRelationWord,$aSearchType='id' )
	{
		...
		if( count($or)>0) { // 各関係条件の検索
			...
			$table = $db->getTable();
			$table = $db->andTable($aTable, $db->searchTableSubQuery( $table, 'id', 'in', $relation) );
		}
		else {
			$table = $aTable;
		}
	...
	static function relationSearch( $aWord, $aRelationType, $aSearchType )
	{
		if( $aSearchType == 'word' ) {
	...
	static function ccEscape( $text ) { return str_replace( " ", "\ ", str_replace(  "/", "\/", $text ) ); }
}
?>