<?php
	...
	class clipSystem extends System
	{
		...
		{
			$c_id = "";
			if( isset( $_GET["c_id"] ) && $_GET["c_id"]!="") { $c_id = $_GET["c_id"]; }
			
			$db		 = $gm[ $_GET['type'] ]->getDB();
			$clip	 = new mod_Clip();

			$idList = array();
			if( $loginUserType != 'admin' && $c_id=="" )	
			{
			...
			}elseif( $loginUserType != 'admin' && in_array($_GET["pal"],array("mid","fresh")) && !empty($c_id)){
			
				$owner_id=SystemUtil::getTableData($_GET["pal"],$c_id,"owner");
