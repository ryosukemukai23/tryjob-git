<?php
	...
	class Command extends CommandBase
	{
		...
		function base_tag( &$gm, $rec, $args ){
			...
			{
				$url = $HOME;
				if( isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on' ) { $url = str_replace ( 'https://', 'https://', $HOME ); }
		...
		*/
		...
		function saveRedirectPage( &$gm, $rec, $args ){
			...
			$notAdmin = "";
			if( isset($args[0]) ) { $notAdmin = $args[0]; }
		...
		function getDataName( &$gm, $rec, $args )
		{
			...
			for( $i=0; $i<count($tgm); $i++ ){
				...
				$table = $db->getTable();
				$table	 = $db->searchTable( $table, 'id', '=', $args[0] );
		...
		{
			$uri = ( 'on' == $_SERVER[ 'HTTPS' ] ? 'https://' : 'https://' ) . $_SERVER[ 'HTTP_HOST' ] . $_SERVER[ 'REQUEST_URI' ];
			$uri = h( $uri , ENT_QUOTES | ENT_HTML401 );
        ...
        */
        ...
        global $loginUserRank;
            ...
            $db = $tgm->getDB();
			$table = $db->getTable();
            $list = $db->searchTable( $table, $args[1] , '=' , true );
        ...
        */
        ...
        global $loginUserRank;
            ...
            $db = $tgm->getDB();
			$table = $db->getTable();
            $list = $db->searchTable( $table, 'regist' , '>' , time() - ($args[1]*60*60) );
            $row = $db->getRow( $list );
...
//             $this->addBuffer( $element.'="'.$args[0].'"'.$nonumber );
		...
		function linkText( &$gm , $rec , $args )
		{
			...
			$option = "";
			if( isset($args[ 1 ]) ) { $option = $args[ 1 ]; }
			$str = preg_replace( '/(' . $re . ')/' , '<a href="$1" ' . $option. '>$1</a>' , $args[ 0 ] );
...
function groupTableSelectFormMultiReflexive( &$str , $param , $check , $d = 0 , $id = null ){
	...
	$table = $db->getTable();
    if( $id != null ){
        $table = $db->searchTable( $table, $param[$d]['parent'] , '=' , $id );
    }
    $row = $db->getRow( $table );
