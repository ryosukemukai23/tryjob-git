<?php
	...
	class Template
	{
        ...
        static function getTemplate( $usertype , $activate , $target , $label , $owner = null ){
            ...
            if(isset(self::$template_cache[$usertype.$target.$label])){
                if(self::$_DEBUG){ d( "getTemplate() : cache load(".self::$template_cache[$usertype.$target.$label].")\n","template"); d(func_get_args(),"args");}
                return self::$template_cache[$usertype.$target.$label];
            }
            ...
            if( $rec = $tdb->getFirstRecord($table) ){
                self::$template_cache[$usertype.$target.$label] = PathUtil::ModifyTemplateFilePath( $tdb->getData( $rec , 'file' ) );
                if(self::$_DEBUG){ d( "getTemplate() : hit=".self::$template_cache[$usertype.$target.$label]."\n","template"); d(func_get_args(),"args");}

				TemplateCache::Using( self::$template_cache[$usertype.$target.$label] );
        ...
        static function drawTemplate( $gm , $rec , $usertype , $activate , $target , $label , $form = false , $owner = null, $partkey = null, $form_flg = null){
            ...
            $gm->setVariable('user_type', $usertype);
