...
class SQLDatabaseBase implements DatabaseBase
{
	...
	function getRecord($table, $index){
		$rec = null;
		if( $table->cacheed && !is_null($table->cache) ){
			if( $this->_DEBUG )				{ d( 'getRecord() : '.$this->tableName.' load table cache<br/>'."\n", 'sql'); }
			$result = &$table->cache;
		}else if( ! is_null( $this->rec_cache )){
			if( $this->_DEBUG )				{ d( 'getRecord() : '.$this->tableName.' load cache<br/>'."n", 'sql'); }
			$result = &$this->rec_cache;
		}else{
			$table = $this->addPasswordDecrypt( $table );

			if( $this->_DEBUG ){ d( 'getRecord() : '. $table->getString(). '<br/>'."\n", 'sql' ); }
			$result	 = $this->sql_query( $table->getString() );
			$this->rec_cache = &$result;
	...
	function getFirstRecord($table){
		$rec = null;
		if( $table->cacheed && !is_null($table->cache) ){
			if( $this->_DEBUG )				{ d( 'getFirstRecord() : '.$this->tableName.' load table cache<br/>'."\n", 'sql'); }
			$result = &$table->cache;
		}else if( ! is_null( $this->rec_cache )){
			if( $this->_DEBUG )				{ d( 'getFirstRecord() : '.$this->tableName.' load cache<br/>'."\n", 'sql'); }
			$result = &$this->rec_cache;
		}else{
			$table = $this->addPasswordDecrypt( $table );
	 ...
	 * @param $type string 操作対象となるテーブルのtype(normal/delete/all)
	...
	function getDataList($table, $name, $key = 'shadow_id' ){
		if( ! is_null( $this->rec_cache )){
			if( $this->_DEBUG )				{ d( 'getDataList() : '.$this->tableName.' load cache<br/>'."\n", 'sql' ); }
			$result = &$this->rec_cache;
		}else{
		...
		if( isset($this->colType[ $name ]) && $this->colType[ $name ] == 'password' ){
	...
	/**
		@brief   レコードの変更されたカラムを確認して要更新マークを付ける。
		@remarks setRecordでレコードを一括代入している場合は全てのカラムに要更新マークがついてしまっているので、\n
		         そのような場合はupdateRecordの前にこのメソッドを呼び出すことで実際に変更されたカラムのみを更新できます。
	*/
	function collateUpdateColumns( &$rec )
	{
		$originRec = $this->selectRecord( $this->getData( $rec , 'id' ) );
		$columns   = Array();

		foreach( $this->colName as $colName )
		{
			$originData = $this->getData( $originRec , $colName );
			$newData    = $this->getData( $rec       , $colName );
	...
	function updateRecord($rec){
		...
		if( !$rec || $this->getData( $rec, 'delete_time' ) ){return -1;}
		
		//レコードの更新が行なわれていないので、更新しない
		if( !isset( $rec[ '@setColumns' ] ) || !count( $rec['@setColumns'] ) ){return 0;}

		global $LOGIN_ID;
		global $loginUserType;

		if( $this->getData( $rec, 'delete_key' ) ){
			return $this->deleteRecord( $rec );
		}
	...
	*/
	...
	function deleteRecord(&$rec){
		...
		if( $this->getData( $rec, 'delete_time' ) )
		{
	...
	function deleteFile($filename){
		...
		if( strlen( $filename ) && $FileBase->file_exists( $filename ) ){
			...
			if( $FileBase->file_exists($filename) && $FileBase->copy($filename, $delname ) ){
				...
				@SystemUtil::safe_chmod($delname, 0766);
			}
			return $delname;
		}
		return null;
	...
	function setData(&$rec, $name, $val){
		...
		$oldData		 = $this->getData( $rec ,$name );
		$rec[ $name ]	 = $this->sql_convert($val);
		$newData		 = $this->getData( $rec ,$name );

		if( isset($this->colType[ $name ]) && 'fake' != $this->colType[ $name ] )
		{
			if( !isset( $rec[ '@setColumns' ] ) )
				{ $rec[ '@setColumns' ] = Array(); }
	...
	function getNewRecord($data = null){
		...
		for($i=0; $i<count( $this->colName ); $i++){
			...
			if( in_array( $this->colType[ $name ] , $CONFIG_SQL_FILE_TYPES ) ){
				...
				switch( $this->colType[ $name ] ){
					case  'timestamp':
						...
						if( isset( $data[ $name.'_year' ] ) ){
							...
							$h   = !empty($data[ $name.'_hour' ])   ? (int)$data[ $name.'_hour' ]   : 0;
							$min = !empty($data[ $name.'_minute' ]) ? (int)$data[ $name.'_minute' ] : 0;
							$s   = !empty($data[ $name.'_sec' ])    ? (int)$data[ $name.'_sec' ]    : 0;
				...
				if( isset( $data[ $name ] ) ){ // null ではない
					if( is_array( $data[ $name ] ) ){ // 配列である
						if( !empty( $data[ $name ] )){ // 空の配列でない
							$str = '';
					...
					}else if( isset($data[ $name ]) ){ // 空文字でない
						if( is_bool( $data[ $name ] ) ){
							...
							$colExtend = '';
							if(isset($this->colExtend[$name])){
								$colExtend = $this->colExtend[$name];
							}
							$data[ $name ] = GUIManager::replaceString( $data[ $name ], $colExtend, $this->colType[$name] );
							$this->setData( $rec, $name, $data[ $name ] );
						}
					}
				}
	...
	/**
	 * DBが持つテーブルを取得します。
	 * @param $type string table type(normal/delete/all)
	 * @return TableBase テーブルデータ
	 */
	...
	function getRow($table){
		...
		if($this->row >= 0 && $table == $this->row_table ){
			if( $this->_DEBUG ){ d( 'getRow() : load cache<br/>'."\n", 'sql'); }
			return $this->row;
		}
	...
	function existsRow($tbl){
		if($this->row >= 0 && $tbl == $this->row_table ){
			if( $this->_DEBUG ){ d( 'existsRow() : load row cache<br/>'."\n", 'sql'); }
			return $this->row > 0;
		}

		if( ! is_null( $this->rec_cache )){
			if( $this->_DEBUG )				{ d( 'existsRow() : '.$this->tableName.' load recorde cache<br/>'."\n", 'sql'); }
			$result = &$this->rec_cache;
		}else{
		...
		$data = 0;
		if( isset($rec['exists_row_cnt']) ) { $data = $rec['exists_row_cnt']; }
		return $data;
	}
	...
	private function searchTableCore(&$name, &$opp, &$val, &$val2 = null, &$tbl_name = null ){
		...
		else if( 'num_fuzzy' == $opp )
		{
			$opp = 'like';
			$val = '%' . implode( '%' , preg_split( "//u" , preg_replace( '/[^\d]/' , '' , $val ) , -1 , PREG_SPLIT_NO_EMPTY ) ) . '%';
		}
		...
		if( is_array( $val ) ){
			...
			$colType = '';
			if(isset($this->colType[$name])){
				$colType = $this->colType[$name];
			}
			$val = $this->sqlDataEscape($val, $colType);
		}
		if( !is_null( $tbl_name ) ){
			$name = $tbl_name.'.'.$name;
		}
	...
	function toString( $rec , $mode )
	{
		$row         = count( $this->colName );
		$sqlElements = Array();
		$setColumns  = ( isset( $rec[ '@setColumns' ] ) ? $rec[ '@setColumns' ] : Array() );

		for( $i = 0 ; $i < $row ; $i++ ) //全てのカラムを処理
		{
			$name = $this->colName[ $i ];

			if( 'fake' == $this->colType[ $name ] ) //fakeカラムの場合
				{ continue; }

			if( $mode == "UPDATE" ) //更新用のSQLを生成する場合
			{
				if( !in_array( $name , $setColumns ) ) //カラムに要更新マークが付いていない場合
					{ continue; }
			}
			...
			if( is_array( $data ) ) //カラムの値が配列の場合、配列データを / で区切って格納
				{ $value = $this->sqlDataEscape( join( '/' , $data ) , $type ); }
			else //カラムの値が実値の場合
				{ $value = $this->sqlDataEscape( $data , $type ); }

			if( $mode == "UPDATE" ) //更新用のSQLを生成する場合
				{ $sqlElements[] = $name . ' = ' . $value; }
			else //それ以外のSQLの場合
				{ $sqlElements[] = $value; }
		}

		return implode( ' , ' , $sqlElements );
	}
	...
	/**
	 * rowとRecordのcacheを削除します。
	 */
	function cacheReset(){
		if( $this->_DEBUG ){ d( 'cacheReset() : reset <br/>'."\n", 'sql' ); }
		$this->row = -1;
		$this->rec_cache = null;
	}
	...
	private function getModelRecord($rec){
		...
		if( !is_array( $THIS_TABLE_MODEL_CLASS ) )
			{ return $rec; }
...
class TableBase{
	...
	function onCache(){ $this->cacheed = true; }
	function offCache(){ $this->cacheed = false; $this->cache = null; }
}
