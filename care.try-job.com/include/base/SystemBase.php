...
class SystemBase extends command_base {
	...
	function drawTitle(&$gm, $rec, $args) {
		...
		if (strpos($_SERVER['REQUEST_URI'], "info.php") !== false) {// 詳細ページの場合
		} else if (strpos($_SERVER['REQUEST_URI'], "search.php") !== false) {// 検索ページの場合
		}
	...
	function drawCanonical(&$gm, $rec, $args) {
		...
		$type = isset($_GET['type']) ? h($_GET['type']) : '';
		$id = isset($_GET['id']) ? h($_GET['id']) : '';

		$buffer = "";
		if (strpos($_SERVER['REQUEST_URI'], "index.php") !== false) {
			if (isset($_GET["app_controller"])) {
				switch ($_GET["app_controller"]) {
					case 'info':
						$buffer = $HOME . 'index.php?app_controller=' . $_GET["app_controller"] . '&type=' . $type . '&id=' . $id;
						break;
					case 'register':
						$buffer = $HOME . 'index.php?app_controller=' . $_GET["app_controller"] . '&type=' . $type;
						break;
					case 'search':
		...
		} else if (strpos($_SERVER['REQUEST_URI'], "info.php") !== false) {// 詳細ページの場合
			$buffer = $HOME . 'info.php?type=' . $type . '&id=' . $id;
		} else if (strpos($_SERVER['REQUEST_URI'], "regist.php") !== false) {// 検索ページの場合
			$buffer = $HOME . 'regist.php?type=' . $type;
		} else if (strpos($_SERVER['REQUEST_URI'], "search.php") !== false) {// 検索ページの場合
			$param = SystemUtil::arrayOmit($_GET);
		...
		if (strlen($buffer) > 0) {
			$this->addBuffer('<link rel="canonical" href="' . h($buffer, ENT_QUOTES | ENT_HTML401) . '">');
		}
	}

	function drawUserApiToken(&$gm, $rec, $args) {
		global $USER_API_TOKEN_NAME;
		global $loginUserType;

		if (isset($_SESSION[$USER_API_TOKEN_NAME])) {
			$this->addBuffer($_SESSION[$USER_API_TOKEN_NAME]);
		}else if( $loginUserType == "nobody" )
		{
			$_SESSION[ $USER_API_TOKEN_NAME ] = md5( rand() );
			$this->addBuffer($_SESSION[$USER_API_TOKEN_NAME]);
		}
	}
	...
	function registCompCheck(&$gm, &$rec, $loginUserType, $loginUserRank, $edit = false) {
		...
		if (!$edit) {
			//重複登録チェック
			$table = $db->getTable();
			$table = $db->searchTable($table, 'id', '=', $db->getData($rec, 'id'));
	...
	public function drawSearch(&$gm, &$sr, $table, $loginUserType, $loginUserRank) {
	 ...
	 * @param loginUserRank ログインしているユーザの権限
	 */
	public function drawSearchNotFound(&$gm, $loginUserType, $loginUserRank) {
	 ...
	 * @param loginUserRank ログインしているユーザの権限
	 */
	public function drawSearchError(&$gm, $loginUserType, $loginUserRank) {
		header('HTTP/1.0 400 Bad Request');
		Template::drawErrorTemplate();
	}
	 ...
	 * @param loginUserRank ログインしているユーザの権限
	 */
	public function drawInfoError(&$gm, $loginUserType, $loginUserRank) {
	 ...
	 * @param loginUserRank ログインしているユーザの権限
	 */
	public function drawInfo(&$gm, &$rec, $loginUserType, $loginUserRank) {
		switch ($_GET['type']) {
			default:
				// 汎用処理
	 ...
	 */
	public function getTemplateFaled($gm, $lavel, $error_name) {
		// ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
		global $loginUserType;
	...
	public function drawActivateComp(&$gm, &$rec, $loginUserType, $loginUserRank) {
		$gm[$_GET['type']]->draw(Template::getLabelFile('ACTIVATE_DESIGN_HTML'), $rec);
	}

	public function drawActivateFaled(&$gm, &$rec, $loginUserType, $loginUserRank) {
	 ...
	 * @param file ファイル配列
	 */
	public function doFileUpload(&$db, &$rec, $colname, &$file) {
		global $FileBase;
		...
		if (!$delete_flag && isset($file[$colname]['name']) && $file[$colname]['name'] != "") {
			$fileName = $this->systemFileUpload($file[$colname], $colname);
			if ($fileName) {
				$db->setData($rec, $colname, $fileName);
			}
			//TODO: false だった場合の例外処理が必要では？
		} else if (!$delete_flag && isset($_POST[$colname . '_filetmp']) && $_POST[$colname . '_filetmp'] != "" && $FileBase->file_exists($_POST[$colname . '_filetmp'])) {
			$db->setData($rec, $colname, $_POST[$colname . '_filetmp']);
			return;
		} else if (!$delete_flag && isset($_POST[$colname]) && $_POST[$colname] != "" && $FileBase->file_exists($_POST[$colname])) {
	...
	public function systemFileUpload($upFile, $colname) {
		...
		// ディレクトリの指定
		$directory = 'file/tmp/';
		if (mb_strpos($this->fileDir, 'lock') !== false) {
			$directory .= 'lock/';
		}
		if (!is_dir($directory)) {
			mkdir($directory, 0777);
			SystemUtil::safe_chmod($directory, 0777);
		} //ディレクトリが存在しない場合は作成
		// ファイルパスの作成
		...
		if (file_exists($upFile['tmp_name'])) {
			$FileBase->upload($upFile['tmp_name'], $fileName);
		}
	...
	public function doFileDelete(&$gm, &$rec, &$old_rec = null) {
	 ...
	 * @param rec レコードデータ
	 */
	public function uplodeComp(&$gm, &$db, &$rec) {
		global $FileBase;
		global $FORCE_RESIZE;
		...
		foreach ($db->colName as $colum) {
			if(!isset($_gm->colType[$colum])){
				continue;	// shadow_id、delete_key は colType に存在しない
			}
			if ($_gm->colType[$colum] == 'image' || $_gm->colType[$colum] == 'file') {
				$before = $db->getData($rec, $colum);
				$after = $before;
				if(!is_null($before)){
					$after = preg_replace('/(file\/tmp\/|file\/tmp\/lock\/)(\w*\.\w*)$/', '\2', $before);
				}
				if ($before != $after) {// ファイルのアップロードが行われていた場合データを差し替える。
					// 拡張子の取得
							...
							case 'cat': // 種類別
								switch ($ext) {
									...
									case 'png':
										$cat = 'image';
										if ($FORCE_RESIZE) {
											$resize = property_exists($this, 'resizedMaxSize') ? $this->resizedMaxSize : 640;
											SystemUtil::resizeImageEx($before, $resize);
										}
										break;
									case 'swf':
										$cat = 'flash';
	 ...
	 * @param args コマンドコメント引数配列です。　第一引数にIDを渡します。 第二引数にリンクするかを真偽値で渡します。
	 */
	public function searchResult(&$gm, $rec, $args) {
		// ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
		global $loginUserType;
		global $loginUserRank;
		...
		$resultNumLocal = (isset($_GET['num']) && 0 < $_GET['num']) ? $_GET['num'] : $resultNum;
...
//				}
	 ...
	 * @param args コマンドコメント引数配列です。　第一引数にIDを渡します。 第二引数にリンクするかを真偽値で渡します。
	 */
	public  function searchCreate(&$gm, $rec, $args) {
		// ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
		global $loginUserType;
		global $loginUserRank;
		// **************************************************************************************
	...
	public function getEmbedSearchResult(&$_gm, $table, $loginUserType, $loginUserRank) {
		global $gm;
	 ...
	 * @param partkey 分割キー
	 */
	public function getSearchPageChange(&$gm, $table, $loginUserType, $loginUserRank, $row, $pagejumpNum, $resultNum, $_phpName, $param) {
		// ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
		global $phpName;	// ※引数と名前が被ってるので、引数を変更。処理ではこちらを使う
		// **************************************************************************************
	 ...
	 * @param loginUserRank ログインしているユーザの権限
	 */
	public function getSearchInfo(&$gm, $table, $loginUserType, $loginUserRank) {
		$type = SearchTableStack::getType();
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	//main css output
	public function css_load(&$gm, $rec, $args) {
		global $css_name;
		global $CSS_PATH;
		...
		if (is_file($CSS_PATH . $css_name)) {
			switch ($loginUserType) {
				...
				default:
					...
					if ($sp_mode) {
						if (is_file($CSS_PATH . 'sp/' . $css_name)) {
							$file = $CSS_PATH . 'sp/' . $css_name;
						} elseif (is_file($CSS_PATH . 'sp/standard.css')) {
							$file = $CSS_PATH . 'sp/standard.css';
						}
					}
	...
	//main js output
	public function js_load(&$gm, $rec, $args) {
			...
			if (isset($root_path[$type]) || is_array($root_path[$type])) {
				foreach ($root_path[$type] as $js_file_path) {
					...
					if (is_array($js_file_path)) {
						$src = $js_file_path['src'];
						$defer = isset($js_file_path['defer']) ? $js_file_path['defer'] : false;
						$async = isset($js_file_path['async']) ? $js_file_path['async'] : false;
					} else {
						$src = $js_file_path;
					}

					if (strpos($src, 'http') === 0 || strpos($src, '//') === 0) {
						$this->addBuffer('<script type="text/javascript" src="' . $src . '"' . ($defer ? ' defer' : '') . ($async ? ' async' : '') . '></script>' . "\n");
					} else {
						// パラメータ付きの場合
						if(strpos($src,'?')!==false){
							$buf = explode('?',$src);
						...
						$ts = @filemtime($file);
						$this->addBuffer('<script type="text/javascript" src="' . $src . '?' . $ts . '"' . ($defer ? ' defer' : '') . ($async ? ' async' : '') . '></script>' . "\n");
					}
				}
			}
		}
	...
	public function feed_load(&$gm, $rec, $args) {
		global $HOME;
		global $CONF_FEED_ENABLE;
		global $CONF_FEED_TABLES;
	...
	//main link output
	public function link_load(&$gm, $rec, $args) {
	...
	public function validate(&$gm, $rec, $args) {
	...
	public function is_validate(&$gm, $rec, $args) {
		foreach (explode('/', $args[0]) as $l) {
			...
			$ret = self::$checkData->isError($l, $data);
			if (strlen($ret)) {
				$this->addBuffer($ret);
				break;
			}
	 ...
	 */
	public function manageExceptionView($className) {
		global $gm;
		global $loginUserType;
		global $loginUserRank;
	...
	public function setErrorMessage(&$gm) {
		if (self::$checkData && !self::$checkData->getCheck()) {
			$gm->setVariable('error_msg', self::$checkData->getError(), false);
			$this->error_msg = "";
		} else {
	 ...
	 * ページに関連付いたレコードを記憶する
	 * @param db Databaseオブジェクト
	 * @param table_type テーブルタイプ a(all)/n(normal)/d(delete)
	 */
	public static function setPageRecord($db, $table_type) {
		global $loginUserType;
		global $LOGIN_ID;
	 ...
	 * @param table_type テーブルタイプ a(all)/n(normal)/d(delete)
	 */
	public function setPreviewRecord($db, $post, $previewMode) {
		global $loginUserType;
		global $loginUserRank;
		global $LOGIN_ID;
		global $gm;
	...
	static function getHead($gm, $loginUserType, $loginUserRank) {
		...
		if (self::$head || isset($_GET['hfnull'])) {
			return "";
		}
		...
		if ($loginUserType == $NOT_LOGIN_USER_TYPE) {
			$html = Template::getTemplateString($gm['system'], null, $loginUserType, $loginUserRank, '', 'HEAD_DESIGN');
		} else {
			$html = Template::getTemplateString($gm[$loginUserType], $rec, $loginUserType, $loginUserRank, '', 'HEAD_DESIGN');
		}

		if (!empty($_SESSION['ADMIN_MODE']) || $loginUserType == 'admin') {
	...
	static function getFoot($gm, $loginUserType, $loginUserRank) {
		...
		if (self::$foot || isset($_GET['hfnull'])) {
			return "";
		}
		...
		if ($loginUserType == $NOT_LOGIN_USER_TYPE) {
			return Template::getTemplateString($gm['system'], null, $loginUserType, $loginUserRank, '', 'FOOT_DESIGN');
		} else {
			return Template::getTemplateString($gm[$loginUserType], $rec, $loginUserType, $loginUserRank, '', 'FOOT_DESIGN');
		}
	}
	...
	static function flush() {
		...
		if (strlen(self::$keywords) > 0) {
			$output = preg_replace('/<meta name="keywords" content=(.*)>/', '<meta name="keywords" content="' . self::$keywords . '">', $output);
		}

		if (strlen(self::$robots) > 0) {
			$output = preg_replace('/<meta name="robots" content=(.*)>/', '<meta name="robots" content="' . self::$robots . '">', $output);
		}
	...
	public function setTitle(&$gm, $rec, $args) {
		self::$title = self::convertSpace($args);
	}

	public function setDescription(&$gm, $rec, $args) {
		self::$description = self::convertSpace($args);
	}

	public function setKeywords(&$gm, $rec, $args) {
		self::$keywords = self::convertSpace($args);
	}

	public function setRobots(&$gm, $rec, $args) {
		self::$robots = self::convertSpace($args);
	}

	public function setOGTitle(&$gm, $rec, $args) {
		self::$ogTitle = self::convertSpace($args);
	}

	public function setOGType(&$gm, $rec, $args) {
		self::$ogType = self::convertSpace($args);
	}

	public function setOGDescription(&$gm, $rec, $args) {
		self::$ogDescription = self::convertSpace($args);
	}

	public function setOGURL(&$gm, $rec, $args) {
		self::$ogURL = self::convertSpace($args);
	}

	public function setOGImage(&$gm, $rec, $args) {
		self::$ogImage = self::convertSpace($args);
	}

	public function convertSpace($text) {
		if (is_array($text)) {
			$text = implode(" ", $text);
		}
	...
	public function modifyTemplateLabel($iTemplateLabel) {
