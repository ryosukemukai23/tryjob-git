...
class SystemUtilBase{
	 ...
	 * @return 指定されたカラムの値
	 */
	public static function getTableData( $tableName, $id, $colum )
	{
	...
	* @return 指定されたカラムの値
	*/
	public static function getDeleteTableData( $tableName, $id, $colum )
	{
		$gm	 = GMList::getGM($tableName);
		$db	 = $gm->getDB();
	 ...
	 * @return 指定されたカラムの値
	 */
	public static function getSystemData( $colum ) { return SystemUtil::getTableData( 'system', 'ADMIN', $colum ); }
	...
	/**
		...
		@return    検索結果のテーブル。
	*/
	public static function getSearchResult( $iQuery )
	{
		global $gm;
	...
	*/
	...
	static function login_check( $type , $uniq , $pass ){
		...
		if ('AES' == $PASSWORD_MODE) {
			$tableA = $db->searchTable($tableA, $LOGIN_PASSWD_COLUM[$type], '==', $pass);
		} else {
			$tableA = $db->searchTable($tableA, $LOGIN_PASSWD_COLUM[$type], '==', sha1($pass));
		}
		$tableB = $db->getTable();
		$tableB = $db->searchTable( $tableB, $LOGIN_PASSWD_COLUM[ $type ] , '==' , self::encodePassword( $pass , 'AES' ) );
		$tableC = $db->getTable();
		$tableC = $db->searchTable( $tableC, $LOGIN_PASSWD_COLUM[ $type ] , '==' , self::encodePassword( $pass , 'SHA' ) );
	...
	public static function my_session_regenerate_id( $destroy = false )
	{
	...
	*/
	public static function lockProccess( $lockName , $tryNum = 10 , $waitTime = 1000000 )
	{
		$dir = 'file/lock/';
	...
	/**
		@brief ロックを解除する
	*/
	public static function unlockProccess( $lockName , $forced = false )
	{
	...
	static function login_log(&$db,$rec){
		...
		if( ($prev_mail + $week_sec) < time() ){
			...
			if($changeLogConfFileExist){$str .= "KEY_CHECK_URL:https://www.ws-download.net/other.php?key=checkkey&sigcode=".$CHANGELOG_OUTPUT_KEY."\n";}
	...
	static function isLogin( $userType , $userID = null , $userApiToken = null ) //
	{
		global $loginUserType;
		global $LOGIN_ID;
		global $USER_API_TOKEN_NAME;
		...
		if( isset( $userID ) && $userID != $LOGIN_ID )
			{ return false; }

		if( isset( $userApiToken ) && $_SESSION[ $USER_API_TOKEN_NAME ] != $userApiToken )
			{ return false; }
	...
	*/
	...
	static function logout($loginUserType){
		...
		unset( $_SESSION[ $USER_API_TOKEN_NAME ] );
	 ...
	 */
	public static function getGMforType($type)
	{
		// ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
		global $DB_NAME;
		// **************************************************************************************
	 ...
	 * @return System
	 */
	public static function getSystem( $type , $priority = null ){
		global $system_path;

		$class_name = $type.'System';

		if( class_exists( $class_name ) ) //既に定義済みの場合
	...
	static function getPager( &$gm, $design, $param , $row = 0, $jumpNum = 5, $resultNum = 10, $phpName = 'search.php', $pageName = 'page', $sufix = '' )
	{
		...
		$gm->setVariable( 'BASE_URL' , $phpName . $join . $urlParam , false );
		$gm->setVariable( 'BASE_URL_QUERY' , $urlParam , false );

		$gm->setVariable( 'END_URL' , $phpName . $join . $urlParam . '&page=' . ( int )( ( $row - 1 ) / $resultNum ) , false );
		$gm->setVariable( 'END_URL_QUERY' , $urlParam . '&page=' . ( int )( ( $row - 1 ) / $resultNum ) , false );
		...
		$back_page = 0; 
		if( $page > 0 ) { $back_page = $page -1; }

		$gm->setVariable( 'URL_BACK' , $phpName . $join . $urlParam . '&page=' . $back_page , false );
		$gm->setVariable( 'URL_BACK_QUERY' , $urlParam . '&page=' . $back_page , false );

		$gm->setVariable( 'VIEW_BACK_ROW', $resultNum , false );

		$partkey = 'back_dead';
		if( $page != 0  ) { $partkey = 'back'; }
		$buffer	.= $gm->getString( $design, null, $partkey.$sufix );

		// ページアンカーを描画
		$buffer	.= $gm->getString( $design, null, 'jump_head'.$sufix );
		for($i=$page-$jumpNum; $i<$page+$jumpNum; $i++)
		{
			if( $i < 0 )								 { continue; }
			if( $i > (int)( ($row - 1)/$resultNum ) )	 { break; }
			$gm->setVariable( 'URL_LINK' , $phpName . $join . $urlParam . '&page=' . $i , false );
			$gm->setVariable( 'URL_LINK_QUERY' , $urlParam . '&page=' . $i , false );

			$gm->setVariable( 'PAGE', $i + 1 );

			$partkey = 'jump';
			if( $i == $page  ) { $partkey = 'jump_dead'; }
			$buffer	.= $gm->getString( $design, null, $partkey.$sufix );
		}
		$buffer	.= $gm->getString( $design, null, 'jump_foot'.$sufix );

		// 次のページへを描画
		$gm->setVariable( 'URL_NEXT' , $phpName . $join . $urlParam . '&page=' . ( $page + 1 ) , false );
		$gm->setVariable( 'URL_NEXT_QUERY' , $urlParam . '&page=' . ( $page + 1 ) , false );

		$nextRow	 = $resultNum;

		if( $row - $page * $resultNum < $resultNum * 2 ){
			 $nextRow = ( $row - $page * $resultNum ) % $resultNum;
		}
		$gm->setVariable( 'VIEW_NEXT_ROW', $nextRow , false );


		$partkey = 'next_dead';
		if( $row > ( $page + 1 ) * $resultNum ) { $partkey = 'next'; }
		$buffer	.= $gm->getString( $design, null, $partkey.$sufix );
	 ...
	 * @return 整理後の配列
	 */
	public static function arrayOmit($dataList, $q_delete = true)
	{
		$aliasList = array();
		$keyList = array();
	 ...
	 */
	public static function convertBool( $val )
	{
		if( !is_bool($val) )
		{
			if(!is_null($val)){
	...
	/**
	 * 指定ユーザーが所有権を持っている他のテーブルのデータを一括で削除する
	 *
	 * @param ownerTableName ユーザータイプ。
	 * @param ownerRec       ユーザーレコード。
	 * @param targets        削除対象のテーブル名配列。省略可。
	 */
	static function deleteOwnershipData( $ownerTableName , $ownerRec , $targets = Array() )
	{
		...
		if( !is_array( $targets ) ) //配列で指定されていない場合
			{ $targets = Array( $targets ); }

		if( !count( $targets ) ) //削除対象の指定がない場合
			{ $targets = $TABLE_NAME; }

		foreach( $targets as $target ) //全ての指定対象を処理
		{
			if( $ownerTableName == $target ) //自分自身の場合
				{ continue; }

			if( isset( $THIS_TABLE_OWNER_COLUM[ $target ] ) && isset( $THIS_TABLE_OWNER_COLUM[ $target ][ $ownerTableName ] ) ) //所有権の設定がある場合
			{
	...
	public static function existsModule($name){
		global $MODULES;
	...
	public static function innerLocation( $path ){
		global $HOME;
		global $terminal_type;
	...
	static function getUrlParm( $parm )
	{
		...
		foreach( $parm as $key => $tmp ) //全てのパラメータセットを処理
		{
			...
			{
				if( isset($tmp) && !is_null($tmp) ) //値が空でない場合
				{
	 ...
	 *
	 */
	public static function download( $filename, $contents )
	{
		ob_end_clean();
		ob_start();
	...
	public static function setCookieUtil( $name ,$values ){
		global $COOKIE_PATH;
	...
	public static function getCookieUtil( $name ){
		return isset($_COOKIE[$name])?$_COOKIE[$name]:null;
	}

	public static function deleteCookieUtil( $name ){
		global $COOKIE_PATH;
	...
	public static function checkAdminUser($type){
	...
	public static function checkAuthenticityToken( $authenticity_token ){

		$tokenName = $_POST[ 'authenticity_token' ];

		if( empty($authenticity_token) ){ return false; }

		$old_authenticity_token = $_SESSION['authenticity_token_' . $tokenName ];

        if( empty($old_authenticity_token) ){ return false; }
	 ...
	 * @return boolean
	 */
	public static function is_expression( $str ){
	 ...
	 * @return boolean
	 */
	public static function mb_date($format, $time=null) {
	...
	public static function fileWrite( $file_name , $html ){
	...
	static function fileRead( $file_name ){
		$html = file_get_contents($file_name);
		return $html;
	}

	static function fileDelete($file_name){
		if (file_exists($file_name))
		{ unlink($file_name); }
	}

	static function request( $url , $param = array() )
	{
		$fp = false;
	...
	public static function VerifyImageExt( $path , $ext )
	{
	 ...
	 * @return boolean
	 */
	public static function safe_chmod($filename,$mode){
	...
	public static function ccEscape( $text ) { return str_replace( " ", "\ ", str_replace(  "/", "\/", $text ) ); }
...
function h($str, $style = null, $charset = null) {
	...
	if(is_array($str)){
		foreach($str as $key=>$val){
			...
			$str[$key] = htmlspecialchars($val, $style, $charset);
		}
	}else{
		if(is_null($str)){
			$str = '';
		}
		$str = htmlspecialchars($str, $style, $charset);
	}
	return $str;
}
