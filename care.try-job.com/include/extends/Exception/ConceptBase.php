	...
	class ConceptBase
	{
		...
		*/
		static function OrThrow()
		{
			...
			{
				// List( $iClassName_ , $iMessage_ ) ;
				$param = func_get_args();
				$iClassName_ = isset($param[0]) ? $param[0]:null;
				$iMessage_ = isset($param[1]) ? $param[1]:null;
		...
		*/
		...
		{
			if( !self::$Instance ) //インスタンスが生成されていない場合
				{ self::$Instance = new ConceptBase(); }
