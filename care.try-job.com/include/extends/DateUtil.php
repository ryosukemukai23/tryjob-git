...
class DateUtil
{
	 ...
	 * @return Unixtime
	 */
	static function getUnixtime( $year = null, $month = null, $day = null, $flg = 'start' )
	{
		$time	 = 0;
	 ...
	 * @return unixtime。
	 */
	static function getYearTime( $time = null, $mode = 'start' )
	{
	 ...
	 * @return unixtime。
	 */
	static function getMonthTime( $time = null, $mode = 'start' )
	{
		if( !isset($time) ) { $time	 = time(); }
	 ...
	 * @return unixtime。
	 */
	static function getWeekTime( $time = null, $mode = 'start' )
	{
		if( !isset($time) ) { $time	 = time(); }
	 ...
	 * @return unixtime。
	 */
	static function getDayTime( $time = null, $mode = 'start' )
	{
		if( !isset($time) ) { $time	 = time(); }
	 ...
	 * @return table
	 */
	static function setSearchYear($db, $table, $y, $col = 'regist')
	 ...
	 * @return table
	 */
	static function setSearchMonth($db, $table, $y, $m, $col = 'regist')
	 ...
	 * @return table
	 */
	static function setSearchDay($db, $table, $y, $m, $d, $col = 'regist')
	 ...
	 * @return table
	 */
	static function setSearchTerm($db, $table, $col = 'regist', $param = array())
	{
		...
		foreach( $modeList as $mode )
		{
			...
			{
				if( !strlen($param[ $col.'_'.$mode.'_'.$check ]) ) { $flg = false; break; }
			}

			if($flg)
			{
				$time[$mode] = self::getUnixtime( $param[$col.'_'.$mode.'_year'], $param[$col.'_'.$mode.'_month'], $param[$col.'_'.$mode.'_day'], $mode );
			}
		}
