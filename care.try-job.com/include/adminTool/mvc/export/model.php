			...
			foreach( $mainStatement as $row ) //全てのレコードを処理
			{
				...
				foreach( $csv->getcolumns() as $column => $option )
				{
					if( 'boolean' == $option[ 'type' ] )
						{ $putData[ $column ] = $row[ $column ] ? 1 : 0; }
					else if( 'password' == $option[ 'type' ] )
						{ $putData[ $column ] = EncodePassword( $row[ $column ] , $PASSWORD_MODE ); }
					else
						{ $putData[ $column ] = $row[ $column ]; }
				}

				fputcsv( $mainFP , $putData );
			}
