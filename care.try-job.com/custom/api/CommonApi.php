<?php
	...
	class mod_CommonApi extends apiClass
	{
		...
		function getChildJsonData( &$param )
		{
			...
			if( !empty($param['noneFlg']) ) { $noneIndex = ''; }

			if( empty($param['parent']) ) { print '{"":"'.$noneIndex.'"}';return; }

			$db	 = GMList::getDB($param['tableName']);
			$table = $db->getTable();
			$table = $db->searchTable( $table, $param['parentCol'], '=', '%'.$param['parent'].'%' );
			$countList = null;
			if( !empty($param['countCol']) )
			{
				switch($param['type'])
				{
				case 'buy':
		...
		*/
		...
		function sendValidity(&$param)
		{
			...
			for ($i = 0; $i < $max; $i ++) {
				if (isset($THIS_TABLE_IS_USERDATA[$TABLE_NAME[$i]])
				&& $THIS_TABLE_IS_USERDATA[$TABLE_NAME[$i]]) {
