<?php
	...
	*/
	class AppnUserRegisterModel extends AppRegisterModel //
	{
		//■処理 //
		...
		*/
		function doRegister() //
		{
			...
			if($this->nobodyRegistValid()){
				nobodyLogic::registResume($this->db->getData($this->rec,"id"),$this->nbRec);
		...
		*/
		function checkValidity(){
			...
			$_GET["mail"] = $val;
			Concept::IsNotEmpty($val)->OrThrow("IllegalAccess");
		}
		...
		*/
		...
		function nobodyRegistValid(){
			if(class_exists("mod_nobody"))
				if(!empty($_SESSION[nobodyLogic::$registSessionName])){
					{ return true;; }
			}
			return false;
		}
		...
		*/
		...
		{
			if (isset($_GET['cp']) 
			&& $this->sys->copyCheck($gm, $this->loginUserType, $this->loginUserRank)) //コピー元の指定がある場合
				{ $this->rec = $this->db->selectRecord( $_GET[ 'cp' ] ); }
			elseif($this->nobodyRegistValid())
				{ nobodyLogic::registInit($_SESSION["nbID"]); }
			else //コピー元の指定がない場合
		...
		*/
		function __construct() //
		{
			...
			if($this->nobodyRegistValid()){
				...
				$this->nbRec = $nbDB->selectRecord($_SESSION["nbID"]);
			}
		}
