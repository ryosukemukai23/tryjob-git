...
class JobView extends command_base
{
	...
	*/
		...
		foreach( $kindList as $kind )
		{
			switch($kind){
				case "prefectures":
					...
					foreach( $areaList as $area_id => $area_name )
					{
						...
						if (isset($addsList[$area_id]))
						{
							...
							{
								if( !$noneDisp && !isset($countList[$id])) { continue; }
								if($countList[$id] == 0){ continue; }
				...
				case 'foreign':
					...
					if( $noneDisp || isset($countList['foreign']) && $countList['foreign'] != 0 ) {
	...
	static function getAddsList( &$gm, $design, $noneDisp, $area )
	{
		// ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
		global $loginUserType;
		// **************************************************************************************
