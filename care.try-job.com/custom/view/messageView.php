	...
	function getItemsList(&$_gm, $_rec, $_args){
		...
		if($type == "mid" || $type == "both"){
			...
			for($i=0;$i<$row;$i++){
				...
				$name =	str_replace( ' ' , '!CODE101;', mb_substr( str_replace( array('!CODE001;','!CODE101;'), ' ' , $name ), 0 , 20, $SYSTEM_CHARACODE ) );
				$cc = '<option value="'.$id.'" '.$selected.'>【 '.$id.' 】　'.$name.'</option>';
				$buffer.='<option value="'.$id.'" '.$selected.'>【 '.$id.' 】　'.$name.'</option>';
			}
