...
class CheckData extends CheckDataBase
{
    ...
    public function checkNullFile($name, $args)
    {
        ...
        if ( isset($_FILES[$name]) && $_FILES[$name]['error'] == UPLOAD_ERR_NO_FILE) {
			// 一時保存画像があるか
            if (isset($_POST[$name . '_filetmp'])) {
				$ft = strlen($_POST[$name . '_filetmp'])>0 ? $_POST[$name . '_filetmp']:null;
			}
			// 一時保存画像も無ければエラー
			if(!isset($ft)){
    ...
    public function checkIsApply4nb($name, $args)
    {
        ...
        if (isset($mail)) {
            ...
            for ($i = 0; $i < $max; $i++) {
                if ( isset($THIS_TABLE_IS_USERDATA[$TABLE_NAME[$i]]) && $THIS_TABLE_IS_USERDATA[$TABLE_NAME[$i]]) {
