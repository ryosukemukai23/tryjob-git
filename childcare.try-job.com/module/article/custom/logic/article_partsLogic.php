<?PHP
	...
	class article_partsLogic
	{
		...
		public static function checkPartsData( $param , $edit=false) {
		...
		public static function headSetParts( &$db, &$rec, $param, $errorfile ) {
			$check = [];
			$check['status'] = 'error';
			$oldRec = $rec;
		...
		public static function textSetParts( &$db, &$rec, $param, $errorfile ) {
			$check = [];
			$check['status'] = 'error';
			$oldRec = $rec;
			$_gm = GMList::getGM(articleLogic::$sub_type);
		...
		public static function linkSetParts( &$db, &$rec, $param, $errorfile  ) {
			global $FileBase;
		...
		public static function imageSetParts( &$db, &$rec, $param, $errorfile  ) {
			global $FileBase;
		...
		public static function moveSetParts( &$db, &$rec, $param, $errorfile  ) {
			$check = [];
			$check['status'] = 'error';
			$oldRec = $rec;
			$_gm = GMList::getGM(articleLogic::$sub_type);
		...
		// 未実装
		public static function goodsSetParts( &$db, &$rec, $param, $errorfile  ) {
			global $FileBase;
		...
		public static function tweetSetParts( &$db, &$rec, $param, $errorfile  ) {
			$check = [];
			$check['status'] = 'error';
			$oldRec = $rec;
			$_gm = GMList::getGM(articleLogic::$sub_type);
		...
		public static function quoteSetParts( &$db, &$rec, $param, $errorfile  ) {
			$check = [];
			$check['status'] = 'error';
			$oldRec = $rec;
			$_gm = GMList::getGM(articleLogic::$sub_type);
		...
		public static function htmlSetParts( &$db, &$rec, $param, $errorfile ) {
			global $loginUserType;
		...
		public static function CheckParts( $param ) {
			$check = [];
			$check['status'] = 'error';
			return $check;
		}

		public static function draw( $data , $template, $edit=false) {
		 ...
		 */
		public static function delete( $iOriginalId )
		{
		 ...
		 * @param userDelete
		 */
		public static function userDelete(&$gm, &$loginUserType,$LOGIN_ID)
		{
			$subDb = $gm[articleLogic::$type]->getDB();

			$table = $subDb->getTable();
			if( $loginUserType == 'admin' ){
				$table = $subDb->searchTable( $table, 'owner', '=', $_GET['id']  );
			}else{
				$table = $subDb->searchTable( $table, 'owner', '=', $LOGIN_ID  );
			}
			...
			if(count($dataList) > 0)
			{
				...
				$table = $db->getTable();
				$table = $db->searchTable( $table, 'original_id', 'in', $dataList );
		 ...
		 */
		public static function getDrawParts( &$gm, $loginUserType, $loginUserRank, $original_id, $partsList, $resultNum, $pagejumpNum, $mode="")
		{
		 ...
		 * ファイルアップロードの完了処理。
		 */
		static function uploadComp( $id, $name )
		{
			global $FileBase;
		...
		public static function getPartsImage($iArec)
		{
			$db = GMList::getDB(articleLogic::$type);
		 ...
		 */
		public static function checkLinkKeywordSearch()
		{
		 ...
		 *  リンクアイテムのキーワード検索が可能か確認する
		 */
		public static function checkImageKeywordSearch($type=null)
		{
			if($type != null)
			{
				// 指定のものが設定されているか確認
		 ...
		 */
		static function checkGoogleSearch()
		{
		 ...
		 * （Twitterアイテムで利用）
		 */
		public static function checkTwitterSearch()
		{
		 ...
		 * （画像アイテムで利用）
		 */
		public static function checkPicPadSearch()
		{
			global $PicPadUrl;
		 ...
		 */
		static function getUrlImage( $iDirectory, $iImagSrc )
		{
			global $FileBase;
...
//					break ;
				...
				elseif( preg_match( '/file\/parts\/tmp\//', $iImagSrc) )
				{ // 一時フォルダにあるものを移動
					$oidfile = $iImagSrc;
					$newfile = str_replace('file/parts/tmp/', $iDirectory, $iImagSrc);
