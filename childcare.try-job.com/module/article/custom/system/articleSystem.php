...
class articleSystem extends System
{
	...
	function editProc(&$gm, &$rec, $loginUserType, $loginUserRank, $check = false)
	{
		...
		$open_y = isset($_POST['open_y']) ? (int)$_POST['open_y'] : date('Y');
		$open_m = isset($_POST['open_m']) ? (int)$_POST['open_m'] : date('n');
		$open_d = isset($_POST['open_d']) ? (int)$_POST['open_d'] : date('j');
		$open_h = isset($_POST['open_h']) ? (int)$_POST['open_h'] : 0;
		$open_i = isset($_POST['open_i']) ? (int)$_POST['open_i'] : 0;
	...
	function drawEditForm(&$gm, &$rec, $loginUserType, $loginUserRank) {
		...
		if ($loginUserType === 'cUser' && $owner !== $LOGIN_ID) {
			Concept::IsFalse(true); // ← 評価を失敗にする
			Concept::OrThrow('IllegalAccess', '他人の投稿を編集する権限がありません');
		}
	...
	function infoCheck( &$gm, &$rec, $loginUserType, $loginUserRank )
	{
		...
		// ▼ポイントチェック追加
		if ($result) {
			$ownerMid = $db->getData($rec, 'owner');
			$currentPoints = PointLogic::getUserPoints($ownerMid);

			// ポイントが1未満なら表示不可にする
			if ($currentPoints < 1) {
				Concept::IsFalse(true);
				Concept::OrThrow('IllegalAccess', 'この記事へのアクセス権限がありません');
			}
		}
		// ▲ここまで追加
	 ...
	 * データから表示可能可確認
	 */
	static function drawCheck( $db, $rec )
	{
		global $ACTIVE_ACCEPT;
	...
	function drawInfo(&$gm, &$rec, $loginUserType, $loginUserRank) {
		...
		// ポイント-1処理を追加（admin・cUser以外の場合のみ）
		if (!in_array($loginUserType, ['admin', 'cUser'])) {
			self::addMinusPointOnView($rec);
		}
	...
	function registCompCheck(&$gm, &$rec, $loginUserType, $loginUserRank, $edit = false) {
		...
		// サムネイルのURL画像のチェック
		if( isset($_POST['url_img']) && strlen($_POST['url_img']) &&  isset($_POST['url_img_used']) &&strlen($_POST['url_img_used']) && !strlen($_POST['image_DELETE']))
		{
			$name = 'image';
	...
	function searchProc( &$gm, &$table, $loginUserType, $loginUserRank )
	{
		...
		default:
			...
			// ▼ここから追加▼
			$table = self::filterByCompanyPoints($db, $table);
			// ▲ここまで追加▲
			break;
		}
	}
	...
	static function drawUserType( $iUser )
	{
		$user = 'nobody';
		switch($iUser)
		{
	...
	function uplodeComp( &$gm, &$db, &$rec )
	{
		...
		{
			if( !isset($gm[$_GET['type']]->colType[$colum]) ) { continue; }
	...
	// ポイントを-1するメソッド
	static function addMinusPointOnView($rec)
	{
		$articleDb = GMList::getDB('article');

		// 記事データ取得
		$articleId = $articleDb->getData($rec, 'id');
		$mid = $articleDb->getData($rec, 'owner');
		$title = $articleDb->getData($rec, 'name');

		// descriptionを作成（記事IDとタイトルを含む）
		$description = "記事(ID:{$articleId})「{$title}」が閲覧されました。";

		// pointテーブルへマイナスポイント追加
		$pointDb = GMList::getDB('point');
		$pointRec = $pointDb->getNewRecord();
		$pointDb->setData($pointRec, 'mid', $mid);
		$pointDb->setData($pointRec, 'point', -1);                    // -1ポイントをセット
		$pointDb->setData($pointRec, 'description', $description);    // 記事IDとタイトルを記載
		$pointDb->setData($pointRec, 'point_type', 'article');        // 固定値として「article」をセット
		$pointDb->setData($pointRec, 'regist', time());               // 現在の時刻をセット

		$pointDb->addRecord($pointRec);
	}

	/**
	 * 企業のポイントが1未満の記事をフィルタリングする
	 *
	 * @param db DBオブジェクト
	 * @param table 元のテーブルデータ
	 * @return フィルター後のテーブル
	 */
	private static function filterByCompanyPoints($db, $table)
	{
		$filteredIds = [];
		...
		for ($i = 0; $i < $row; $i++) {
			...
			$ownerId = $db->getData($rec, 'owner');

			// 企業のポイントを取得（1以上のみ許可）
			$points = PointLogic::getCurrentPoints($ownerId);

			if ($points >= 1) {
				$filteredIds[] = $db->getData($rec, 'id');
			}
		}

		if (empty($filteredIds)) {
			// ポイントがある記事がひとつもない場合、空のテーブルを返す
			return $db->getEmptyTable();
		}

		// 許可された記事IDのみ再取得する
		return $db->searchTable($table, 'id', 'in', $filteredIds);
	}
}