...
class EntryImportLogic extends ImportLogic
{
	...
	function registMessage($base, $userType)
	{
		...
		foreach ($gm->colName as $col) { // 同名カラムが存在する初期値をセット
			if (isset($base[$col]) && strlen($base[$col]) > 0) {
				$db->setData($rec, $col, $base[$col]);
			}
		}
		// 生成したデータをセット
		foreach ($data as $col => $value) {
			$db->setData($rec, $col, $value);
		}
