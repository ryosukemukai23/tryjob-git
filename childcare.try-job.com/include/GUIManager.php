<?php
	...
	class GUIManager
	{
		...
		var $table_type;							// テーブルタイプa(all)/n(normal)/d(delete)
		...
		*/
		 ...
		 */
		function getDB()	{	$this->db->cacheReset();	return $this->db; }
		 ...
		 */
		 function getVariable($name){
			 if(is_null($this->variable) || !isset($this->variable[$name]) ){
				 return null;
			 }
		 ...
		 * @return string
		 */
		function getListNumString($html, $table, $partkey = null,$start=1)
		{
			$buffer	 = "";
			$row	 = $this->db->getRow( $table );
			$table->onCache();
			...
			for( $i=$zMax; $i>0; $i-- )
			{
						...
						if( $stack[ $j ] !== "\0" && strlen( $stack[ $j ] ) > 0  )
						{
                            ...
                            if( $state['draw'] > 0 ){//表示
								if(  strpos( $stack[ $j ], self::$CC_HEAD ) !== false  )	{
									...
									$stack[ $j ] = is_null($stack[ $j ]) ? '':str_replace( array('!CODE001;','!CODE000;',self::$CC_HEAD,self::$CC_FOOT), array(' ', '/', '!CODE005;','!CODE006;') , $stack[ $j ] );
	                            }
				...
				if( count($draw_change) ){
	                ...
	                for( $k=0; $k<count($stack); $k++ )
	                {
	                	...
	                	if( isset($draw_change[$change_point]) && !is_null($draw_change[$change_point]) && $draw_change[$change_point] <= $k )
	                	{
	                		$change_point++;
	                		$draw_b = !$draw_b;
	                	}
