...
class CheckDataBase
{
	...
	function __construct( &$gm, $edit, $loginUserType, $loginUserRank, $type = false )
	{
		...
		$this->gm			 = "";
		if( isset( $gm[ $this->type ] ) )
		{ $this->gm = $gm[ $this->type ]; }
		$this->loadErrorDesign( $loginUserType,$loginUserRank);
		$this->check		 = true;
		$this->error_name  	 = Array();
		$this->error_msg   	 = Array();
		$this->edit			 = $edit;
		$this->design		 = ( isset( $_GET[ 'design' ] ) ? $_GET[ 'design' ] : null );
	}
	...
	function setErrorDesign( $design_file ){
		$this->error_design = array($design_file);
	}
	function addErrorDesign( $design_file ){
		$this->error_design[] = $design_file;
	}
	function loadErrorDesign( $loginUserType, $loginUserRank ){
		global $ADD_LST;
		global $MODULES;
		$this->error_design	 = array(Template::getTemplate( $loginUserType , $loginUserRank , $this->type , 'REGIST_ERROR_DESIGN' ));
		//ADD_LST に付随して追加デザインがある場合はそちらも反映する
		if(!empty($ADD_LST[$this->type]) && count($ADD_LST[$this->type])){
			foreach( $ADD_LST[$this->type] as $modName => $lst_file ){
                if(isset($MODULES[ $modName ])){
					$modErrorDesignFile = array();
					$modErrorDesignFile[] = "[$modName]/$this->type/SearchFailed.html";
					$modErrorDesignFile[] = "[$modName]/$this->type/SearchFaled.csv";
					foreach( $modErrorDesignFile as $error_design ){}
					$error_design = PathUtil::ModifyTemplateFilePath( $error_design );
					if( is_file( $error_design)){
						$this->error_design[] = $error_design;
					}
				}
			}
		}
	...
	function checkFileError( $edit = false ){
		...
		for($i=0; $i<$row; $i++){
			...
			if( array_search( $this->gm->colType[ $name ], $CONFIG_SQL_FILE_TYPES ) !== FALSE && isset( $_FILES[ $name ]) ){
				...
				if(preg_match( '/(\.\w*$)/', $_FILES[ $name ][ 'name' ], $tmp )){
	...
	function checkNull($name,$args)
	{
		...
		if( isset($this->gm->colType[ $name ]) && array_search( $this->gm->colType[ $name ], $CONFIG_SQL_FILE_TYPES ) !== FALSE ){
	...
	*/
	...
	function checkIntable($name,$args)
	{
		...
		if( isset( $this->data[ $name ] ) && $this->data[ $name ] != null )
		{
			$db = $gm[ $type ]->getDB();
			$table = $db->getTable();
			if( !$db->existsRow( $db->searchTable( $table, 'id' , '=' , $this->data[ $name ] ) ) ){
	...
	*/
	...
	function checkUri($name,$args){
		// フォームにチェックすべき値が無い時はチェックを無視する
		if( !empty($this->error_name[$name]) || !isset($this->data[$name])
		|| isset($this->data[$name]) && strlen($this->data[$name])==0
		) {
			return $this->check;
		}
	...
	public static function is_mail($text,$level = 3,$dns_check = false)
	{
		switch($level){
			case 0: default:
				...
				break;
			case 1:
				//https://www.tt.rim.or.jp/~canada/comp/cgi/tech/mailaddrmatch/
			...
			case 4:
				...
				break;
			case 5:
				//Cal Henderson: https://iamcal.com/publish/articles/php/parsing_email/pdf/
	...
	function checkMailDup($name,$args)
	{
		...
		{// メールアドレス重複チェック
			...
			for($i=0; $i<$max; $i++)
			{
				if(  !empty($THIS_TABLE_IS_USERDATA[ $TABLE_NAME[$i] ]) )
				{
	...
	function checkDateNull($name,$args)
	{
		...
		$m_key = $name.'_month';
		$d_key = $name.'_day';
		if( ( !strlen($this->data[$y_key]) || !strlen($this->data[$m_key]) || !strlen($this->data[$d_key]) ) && !strlen($this->data[$name]) ){
			$this->addError($name);
		}
	...
	function checkOlddate($name,$args)
	{
		...
		if( strlen($this->data[$y]) && strlen($this->data[$m]) && strlen($this->data[$d]) ){
			if(!isset($this->error_name[ 'date_format' ])){
				$hour = 0;
				$min = 0;
				if(isset($this->data[$h])) { $hour = $this->data[$h]; }
				if(isset($this->data[$min])) { $hour = $this->data[$min]; }
				if( mktime( $hour , $min ,0,$this->data[$m],$this->data[$d] , $this->data[$y]) < time() ){
	...
	function checkChild($id, $type, $column ){
		...
		$ctable = $cdb->getTable();
		$ctable = $cdb->searchTable( $ctable, $column, '=', $id );
	...
	function generalCheck($edit, $data = false)
	{
		...
		for($i=0; $i<$row; $i++)
		{
			...
			if(isset($this->gm->colCheck[$name])){
				$checkPage = explode("/",$this->gm->colCheck[$name]);
			}else{
				$checkPage = array();
			}
			...
			if( strlen($pal) ){
				...
				foreach( $checks as $check ){
					...
					//design=が未指定ならばチェックしない
					if(!empty($this->design)){
						//不一致ならばスキップ
						if(!in_array($this->design,$checkPage)){
							continue;
						}
					}
	...
	function getError( $label = null )
	{
		...
		{// エラー内容がある場合
			...
			if( strlen($error) )
			{
				$tmp	.= $this->gm->partGetString( $this->error_design[0] , 'head');
				$tmp	.= $error;
				$tmp	.= $this->gm->partGetString( $this->error_design[0] , 'foot');
			}
		}
	...
	function addError($part,$def=null,$name=null ){
		foreach( $this->error_design as $design){
			$str = $this->gm->partGetString(  $design , $part );
			if(strlen($str)){ break; }
		}
		...
		if($this->check) {
			...
			$this->error_msg[ 'is_error' ] .= $this->gm->partGetString(  $this->error_design[0] , 'is_error' );
		}
		$this->check = false;
	...
	function checkMultiFileNull($name,$args)
	{
		...
		if(array_search( $this->gm->colType[ $name ], $CONFIG_SQL_FILE_TYPES ) !== FALSE ) { //ファイル型カラムの場合
			...
			for ($i = 1; $i <= $max; $i++) { //連番名のフォームを処理
				$colname = $multi_colname . $i;

				$isPosted   = ( $_FILES[ $colname ][ 'name' ] || !empty( $this->data[ $colname . '_filetmp' ] ) || !empty( $this->data[ $colname ] ) );
				$notDeleted = ( !isset( $this->data[ $colname . '_DELETE' ] ) || $this->data[ $colname . '_DELETE' ] != "true" );

				if( $isPosted && $notDeleted ) //送信されているファイルがある場合
					{ ++$fileCount; }
			}

			if( isset( $_FILES[$multi_colname]['name'] ) && is_array( $_FILES[$multi_colname]['name'] ) )
			{
