	...
	*/
	class MVC //
	{
		...
		*/
		...
		{
			foreach( Array( 'Model' , 'View' , 'Controller' ) as $loadType ) //目的のクラスを処理
			{
				if( preg_match( '/^App(\w+)' . $loadType . '$/' , $iClassName , $matches ) ) //目的のクラスと一致する場合
				{
					$matches[ 1 ] = strtolower( $matches[ 1 ] );

					foreach( self::GetPackedMVCTypes() as $type ) //全てのパックタイプを処理
					{
						if( preg_match( '/^' . $type . '(\w+)$/' , $matches[ 1 ] , $matchesEx ) ) //パッククラスと一致する場合
						{
							if( is_file( self::$PackedMVCPath . $type . '/mvc/' . strtolower( $matchesEx[ 1 ] ) . '/' . strtolower( $loadType ) . '.php' ) ) //パックファイルがある場合
							{
								include_once self::$PackedMVCPath . $type . '/mvc/' . strtolower( $matchesEx[ 1 ] ) . '/' . strtolower( $loadType ) . '.php';
								return;
							}
						}
					}

					foreach( self::GetPackedMVCPath() as $key => $path ) //全てのパックパスを処理
					{
						if( $key == $matches[ 1 ] ) //パッククラスと一致する場合
						{
							if( is_file( $path . strtolower( $loadType ) . '.php' ) ) //パックファイルがある場合
							{
								include_once $path . strtolower( $loadType ) .'.php';
								return;
							}
						}
					}

					// custom/mvc 以下のテーブル名が小文字じゃなくても
					// ファイルを読むように改造済
					foreach( self::GetExtendMVCTypes() as $type ) //全ての拡張タイプを処理
					{
						if( preg_match( '/^' . strtolower($type) . '(\w+)$/' , $matches[ 1 ] , $matchesEx ) ) //拡張クラスと一致する場合
						{
							if( is_file( self::$ExMVCPath . $type . '/' . strtolower( $matchesEx[ 1 ] ) . '/' . strtolower( $loadType ) . '.php' ) ) //拡張ファイルがある場合
							{
								include_once self::$ExMVCPath . $type . '/' . strtolower( $matchesEx[ 1 ] ) . '/' . strtolower( $loadType ) . '.php';
								return;
							}
						}
					}

					if( is_file( self::$CommonPackedMVCPath . strtolower( $matches[ 1 ] ) . '/' . strtolower( $loadType ) . '.php' ) ) //汎用パッククラスがある場合
					{
						include_once self::$CommonPackedMVCPath . strtolower( $matches[ 1 ] ) . '/' . strtolower( $loadType ) . '.php';
						return;
					}

					if( is_file( self::$MVCPath . $matches[ 1 ] . '/' . strtolower( $loadType ) . '.php' ) ) //汎用クラスがある場合
						{ include_once self::$MVCPath . $matches[ 1 ] . '/' . strtolower( $loadType ) . '.php'; }
		...
		*/
		private static function GetExtendMVCTypes() //
		{
			...
			while( $entry = readdir( $dir ) ) //全てのエントリを処理
			{
				...
				// custom/mvc 以下のテーブル名が小文字じゃなくても
				// ファイルを読むように改造済
//				self::$ExtendMVCTypes[] = strtolower( $entry );
		...
		*/
		...
		/**
			@brief  モジュールパックMVCが存在するパスの一覧を取得する。
			@return パス配列。
		*/
		private static function GetPackedMVCPath() //
		{
			if( !count( self::$PackedMVCPathList ) )
			{
				$entries = glob( self::$PackedMVCPath . '*/mvc/*/*/' );

				foreach( $entries as $entry )
				{
					if( preg_match( '/^\w+\/\w+\/mvc\/(\w+)\/(\w+)\//' , $entry , $matches ) )
						{ self::$PackedMVCPathList[ strtolower( $matches[ 1 ] . $matches[ 2 ] ) ] = $entry; }
				}
			}

			return self::$PackedMVCPathList;
		}

		static private $MVCPath             = 'include/mvc/'; ///<MVCファイルの格納パス。
		static private $ExMVCPath           = 'custom/mvc/';  ///<拡張MVCファイルの格納パス。
		...
		static private $PackedMVCPathList   = Array();        ///<パックMVCのパス一覧。
	}
