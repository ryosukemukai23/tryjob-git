<?php
	...
	class ccProc extends ccProcBase
	{
		...
		public static function arrayValueReplace(&$gm, $rec, $cc)
		{
			$ret = "";
			if( !isset($gm) ) {
		...
		*/
		static function convertString( &$iGM , $iRec , $iArgs ) //
		{
			...
			else //aliasモードの場合
			{
				...
				foreach( $originValue as $value ) //全ての値を処理
				{
					...
					if( isset( self::$alias_cache[ $cacheName ] ) ) //キャッシュがある場合
					{
						$rec      = self::$alias_cache[ $cacheName ];
		...
		static function object(&$gm, $rec, $cc)
		{
			...
			case 'image':
					...
					{// getimagesizeが重いので最大サイズが指定されている場合のみ処理
			...
			*/
			...
			switch($cc[1])
			{
				...
				case 'check':
					...
					if( !isset( $cc[ 8 ] ) || !strlen( $cc[ 8 ] ) || $cc[ 8 ] != 'true' )
						{ $ret .= '<input type="hidden" name="'. $col .'_CHECKBOX" value="" />'."\n"; }
					break;
				...
				case 'multi_file':
					...
					if( isset( $cc[ 8 ] ) && $cc[ 8 ]=='drop'  )
					{
						...
						$ret .= '<div class="drop-zone-multi" data-type="'.$uptype.'" data-upmax="'.$max_num.'" data-name="'.$col.'"><p>ここにファイルをドロップ</p><output></output></div>';
					}
					break;
				case 'image' : //ファイル入力(type=fileのinputタグ)を出力(サムネイル表示付き)
					...
					{
						$ret .= '<div class="drop-zone" data-type="image" data-name="'.$col.'"><p>ここにファイルをドロップ</p><output></output></div>';
					}
				...
				case 'file' : //ファイル入力(type=fileのinputタグ)を出力
					...
					{
						$ret .= '<div class="drop-zone" data-type="file" data-name="'.$col.'"><p>ここにファイルをドロップ</p><output></output></div>';
					}
				...
				case 'time':
					...
					for( $i = 0 ; count( $formats ) > $i ; ++$i )
					{
						...
						// $suffixs[ 1 ]が無いというwarningが山程出るので修正
						if(isset($suffixs[ $i ])){
							$_suffixs = $suffixs[ $i ];
						}else{
							$_suffixs = null;
						...
						{
							$originValue = isset($_POST[ $cc[ 2 ] . '_year' ]) ? $_POST[ $cc[ 2 ] . '_year' ]:null;

							if( 0 == $originValue ) //年の値が0の場合
								{ unset( $_POST[ $cc[ 2 ] . '_year' ] ); }

							$ret .= ccProc::controller( $gm , $rec , array( 'form' , 'text' , $cc[ 2 ] . '_year' , '4' , '4' , $value , $option ) ) . $_suffixs;

							$_POST[ $cc[ 2 ] . '_year' ] = $originValue;
						}
						else if( 'm' == strtolower( $formats[ $i ] ) )
							{ $ret .= ccProc::controller( $gm , $rec , array( 'code' , 'num_option' , $cc[ 2 ] . '_month' , '12' , $value , '1' , '--' , $option ) ) . $_suffixs; }
						else if( 'd' == strtolower( $formats[ $i ] ) )
							{ $ret .= ccProc::controller( $gm , $rec , array( 'code' , 'num_option' , $cc[ 2 ] . '_day' , '31' , $value , '1' , '--' , $option ) ) . $_suffixs; }
						else if( 'h' == strtolower( $formats[ $i ] ) )
							{ $ret .= ccProc::controller( $gm , $rec , array( 'code' , 'num_option' , $cc[ 2 ] . '_hour' , '23' , $value , '0' , '--' , $option ) ) . $_suffixs; }
						else if( 'i' == strtolower( $formats[ $i ] ) )
							{ $ret .= ccProc::controller( $gm , $rec , array( 'code' , 'num_option' , $cc[ 2 ] . '_minute' , '59' , $value , '0' , '--' , $option ) ) . $_suffixs; }
						else if( 's' == strtolower( $formats[ $i ] ) )
							{ $ret .= ccProc::controller( $gm , $rec , array( 'code' , 'num_option' , $cc[ 2 ] . '_sec' , '59' , $value , '0' , '--' , $option ) ) . $_suffixs; }
					}
					break;
		...
		*/
		...
		{
			array_shift( $cc );
			$name = isset($cc[0]) ? $cc[0]:null;
			$index = isset($cc[1]) ? $cc[1]:null;
			
			$ret = "";

			if( isset( $_POST[ $name ] ) && is_array( $_POST[ $name ] ) ) //POSTが配列の場合
			{
				if( !isset( $index ) )
				{ //indexが指定されていない場合
...
*/
		...
		// typeが分からないユーザーIDの情報を出力
		static function user(&$gm, $rec, $cc)
		{
			$id = $cc[1];
			$col = $cc[2];
			$ret = "";
		...
		static function comma( &$gm , $rec , $cc ) //
		{
			...
			$cc  = array_slice( $cc , 1 );
			$ret = ccProc::controller( $gm , $rec , $cc );
			$ret = number_format( floor( (float)$ret ) ) . strstr( $ret , '.' );
		...
		/**
		 * mod_onコマンド。
		 * このコマンドは他のコマンドの前に付ける形で利用する事により、第一引数のクラスが定義されている場合のみ値を出力する。
		 *
		 */
		static function mod_on(&$gm, $rec, $cc)
		{
			$ret  = '';
			$module	 = $cc[1];
			
			if( class_exists($module) || class_exists('mod_'.$module))
			{
				$cc2 = array_slice($cc,2);
				$ret = ccProc::controller($gm, $rec, $cc2);
			}
        ...
        static function ifbegin(&$gm, $rec, $cc)
		{
            ...
            switch( $cc[1] ){
                ...
                case 'get=':
					...
					$check = "";
					if(isset($cc[3])) { $check = $cc[3]; }
                    return isset($_GET[$cc[2]])?($_GET[$cc[2]]==$check):''==$check;
                case 'post_equal':
                    //第二引数をGET引数の連想配列名とした値と、第三引数に指定された値が一致するかどうか。
					$check = "";
					if(isset($cc[3])) { $check = $cc[3]; }
                    return isset($_POST[$cc[2]])?($_POST[$cc[2]] == $check):''==$check;
                case 'uri_match':
