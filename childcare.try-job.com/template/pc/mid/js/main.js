...
$(window).on("resize", initAccordion);
  ...
  // --- 駅関連の処理 ---
  function initializeStationHandlers() {
    // 駅が選択された時
    $('select[name="station"]').on("change", function () {
      const selectedStation = $(this).val();
      $('input[name="station_PAL[]"]').val(selectedStation);
    ...
    // 初期値があれば設定
    const initialStationValue = $('select[name="station"]').val();
    if (initialStationValue) {
      $('input[name="station_PAL[]"]').val(initialStationValue);
    }
  }
