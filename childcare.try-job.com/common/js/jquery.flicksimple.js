 ...
 *
 * Copyright (c) 2011 Makog. https://d.hatena.ne.jp/makog/
	...
	$.extend( $.flickSimple.prototype, {
		...
		//	o.debug = $('#debug');
			...
			if ( typeof window.onorientationchange !== 'undefined' ) {
				$(window).on( 'orientationchange', function(){ o.updateSize(); } );
			} else {
				$(window).on( 'resize', function(){ o.updateSize(); } );
			}
			o.updateSize();
			
			if ( o.touchable ) {
				o.elm.on( 'touchstart', function(e){ o.touchstart(e) } )
					.on( 'touchmove', function(e){ o.touchmove(e) } )
					.on( 'touchend', function(e){ o.touchend(e) } );
			} else {
				o.elm.on( 'mousedown', function(e){ o.touchstart(e) } );
				$('body').on( 'mouseup', function(e){ o.touchend(e) } )
					.on( 'mousemove', function(e){ o.touchmove(e) } );
			}

			o.target.on( 'webkitTransitionEnd', function(e) {
		...
		init: function() {
			...
			o.target.find('a').each( function() {
				...
				if ( ! o.touchable ) {
					obj.off( 'mousedown', o.no_mousedown )
						.on( 'mousedown', o.no_mousedown );
				}
			} );
...
$(document).ready(function(){
	...
	if($(fl).css('width')){
		var fl_id='fl1';
		var fl_width=parseInt($(fl).css('width').replace('px',''),10);
		var fl_child=parseInt($('#'+fl_id).children().length,10);
		var fl_li_width=parseInt($('#'+fl_id+' li').css('width').replace('px',''),10);
		$('#'+fl_id).css('width',fl_li_width*fl_child+'px');
	}
	...
	// next button
	$('#flicknextbtn1').on("click", function() {
		fl.flickSimple( 'nextPage', 1 );
		return false;
	} );
	
	// back button
	$('#flickbackbtn1').on("click", function() {
