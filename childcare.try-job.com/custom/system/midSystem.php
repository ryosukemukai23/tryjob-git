...
class midSystem extends System
{
	...
	function editProc( &$gm, &$rec, $loginUserType, $loginUserRank ,$check = false)
	{
		...
		if( !$plan_select && !$apply && $employment ){
			$db->setData($rec,"term_type","employment");
		}elseif( !$plan_select && $apply && !$employment ){
	...
	function editComp( &$gm, &$rec, &$oldRec, $loginUserType, $loginUserRank )
	{
		...
		JobLogic::AdjustDayEndTime($rec);	// 掲載期限、おすすめ掲載日時調整
		...
		if($old != $new && $new == $ACTIVE_ACCEPT)
			MailLogic::noticeProjectActivate("mid",$rec);
	...
	function feedProc( $table = null )
	{
		...
		if(!Conf::checkData("job", "feed", "mid"))
			return;
	...
	function searchProc( &$gm, &$table, $loginUserType, $loginUserRank )
	{
		...
		if(isset($_GET["pid"]) && SystemUtil::existsModule("special")){
						...
						switch ($db->colName[$i]){
							case "activate":
								if($db->getData($rec, $db->colName[$i]) !== $_POST[$db->colName[$i]]){
									if($_POST["activate"] == $ACTIVE_ACCEPT) MailLogic::noticeProjectActivate("mid", $rec);
								}
								break;
							case "attention":
			...
			case 'search':
				if($STATIC_URL_FLG && SystemUtil::checkOnlySearch($_GET, 
					array(
						...
						array('category', 'work_style'),
						array('category', 'addition'),
						array('work_place_adds', 'work_style'),
						array('work_place_adds', 'work_place_add_sub', 'work_style'),
						array('work_place_adds', 'addition'),
						array('work_place_adds', 'work_place_add_sub', 'addition'),
						array('work_style', 'addition'),
						array('category'),
						array('areaID'),
