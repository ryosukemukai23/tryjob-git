<?php
	...
	class Extension extends ExtensionBase
	{
		...
		function drawAccessCountAll( &$gm, $rec, $args )
		{
			...
			$total= 0;
			if( !is_bool($countData) && is_array($countData[$year][sprintf("%02d",$month)]))
			{ $total =  array_sum($countData[$year][sprintf("%02d",$month)]); }
		...
		function drawAccessCountNum( &$gm, $rec, $args )
		{
			...
			for($i=1;$i<=$dayMax;$i++){
				if( isset($countData[$year][sprintf("%02d",$month)][sprintf("%02d",$i)]) ){
		...
		function getYearly( &$gm, $rec, $args )
		{
			...
			$mode = null;
			if( isset($args[0]) ) { $mode = $args[0]; }

			$this->addBuffer(SystemUtil::getYearly($mode));
		}
		...
		global $css_name;
			...
			$db = $tgm->getDB();
			$table = $db->getTable();
			$table = $db->searchTable( $table, 'label' , '=' , 'CSS_LINK_LIST' );
