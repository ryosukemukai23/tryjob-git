<?php
	...
	class Area
	{
		 ...
		 * @return レコード
		 */
		public static function getAreaRecord( $id )
		{
		...
		public static function getAddsData( $id )
		{
		 ...
		 * @return CC
		 */
		public static function getAddsSelectCC( $colName, $childCol, $lineMode, $countFlg = false, $noneFlg = false, $dispFlg = false, $sortFlg = false )
		{
			$tableName	 = 'adds';
			$initial	 = '';
			...
			if( isset($_GET['pos_word']) && is_array($_GET['pos_word']) && count($_GET['pos_word']) > 0 )
		 ...
		 * @return CC
		 */
		public static function getAddsSelectCCEx( $colName, $stationCol, $lineMode, $countFlg = false, $noneFlg = false, $dispFlg = false )
		{
			$tableName	 = 'adds';
		 ...
		 * @return jsCode
		 */
		public static function createParentJsCode( $colName, $childCol, $childTableName, $childSearchCol, $lineMode, $countFlg, $noneFlg, $dispFlg )
		 ...
		 * @return CC
		 */
		public static function getAddSubSelectCC( $colName, $parentCol, $parent = '', $option = '', $countFlg = false, $dispFlg = false, $sortFlg = false )
		{
			$tableName	 = 'add_sub';
			...
			// nUserフォームなどでは、$_POSTに値がセットされており、
			// $_REQUESTに値は設定されていないので、置き換え不可
			$parent = '';
			if(!empty($_POST[$parentCol])){
				$parent = $_POST[$parentCol];
			}else if(!empty($_GET[$parentCol])){
				$parent = $_GET[$parentCol];
			}
		 ...
		 * @return CC
		 */
		public static function getLineSelectCC( $colName, $childCol, $parentCol, $parent = '', $countFlg = false  )
		{
			$tableName	 = 'line';
			$initial	 = '';
			...
			// 親IDの指定。優先順位 POST>GET>引数
			if( isset($_POST[$childCol]) && strlen($_POST[$childCol]) )	     { $parent = SystemUtil::getTableData($childTableName, $_POST[$childCol], $prefSearchCol); }
			else if( isset($_GET[$childCol]) && strlen($_GET[$childCol]) )  { $parent = SystemUtil::getTableData($childTableName, $_GET[$childCol], $prefSearchCol); }
			else if( isset($_POST[$parentCol]) && strlen($_POST[$parentCol]) ){ $parent = $_POST[$parentCol]; }
			else if( isset($_GET[$parentCol]) && strlen($_GET[$parentCol]) ) { $parent = $_GET[$parentCol]; }
		 ...
		 * @return jsCode
		 */
		public static function createStationJsCode( $colName, $childCol, $childTableName, $childSearchCol, $prefCol, $prefSearchCol, $countFlg )
		 ...
		 * @return CC
		 */
		public static function getStationSelectCC( $colName, $parentCol, $prefCol, $parent = '', $option = '', $countFlg = false )
		{
			$tableName	 = 'station';
