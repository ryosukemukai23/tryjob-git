...
class JobLogic
{
    ...
    public static function getTable($type, $table = null, $param = null, $userType = null)
    {
        ...
        if ($loginUserType == "admin" && !empty($_GET["authority"])) {
        ...
        if (isset($param)) {
            ...
            if ($magic_quotes_gpc) {
				...
				$param_ = addslashes_deep($param);
                $sr->setParamertorSet($param_);
            }
    ...
    public static function searchSystemPublish($db, $table, $param, $userType)
    {
        if ($userType != 'nobody') {
            return $table;
        }
     ...
     * @return void
     */
    public static function setParam($db, &$rec, $type)
    {
    ...
    public static function searchTrain($db, $table, $param)
    {
        ...
        if (isset($param['traffic_station']) && strlen($param['traffic_station'])>0) {
            // 駅が指定されている場合
        ...
        } elseif (isset($param['traffic_line']) && strlen($param['traffic_line'])) {
            // 路線までしか指定されていない場合
    ...
    {
        if (!isset($param['areaID']) || $param['areaID'][0]==null) {
            return $table;
        }
    ...
    public static function searchPublish($db, $table, $param, $userType, $type)
    {
        ...
        case "admin":
        case "cUser":
            if (isset($param['publish_now']) && $param['publish_now'] == "on") {
                $search = true;
            }
            break;
        default:
        ...
        if ($search) {
			$subTable = cUserLogic::getActivateIdTable($userType, $type);
            $table = $db->searchTableSubQuery($table, 'owner', 'in', $subTable);
        }
    ...
    public static function searchHistry($db, $table, $param)
    {
        ...
        if (isset($param['history']) && $param['history'] == "on") {
			$mode = viewMode::getViewMode();
			if(isset($_COOKIE[$mode])){
	            $table = $db->searchTable($table, 'id', 'in', explode(',', $_COOKIE[$mode]));
			}
        }
    ...
    public static function sortTotal($db, $table, $param)
    {
        ...
        if (isset($param['sortEx']) && $param['sortEx'] == 'PV') {
     ...
     * @return boolean $result 応募可能な場合true
     */
    public static function checkApplyNumber($type, $id)
    {
     ...
     * @return boolean $result 応募可能な場合true
     */
    public static function checkLimit($type, $id)
    {
        $rec = Job::getRecord($type, $id);
     ...
     * @return boolean $result 応募可能な場合true
     */
    public static function checkRegistMax($owner)
    {
        $result = true;
     ...
     * @return boolean $result 応募可能な場合true
     */
    public static function checkDisp($type, $rec)
    {
        // ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
        global $loginUserType;
        global $LOGIN_ID;
     ...
     * @return boolean $result 応募可能な場合true
     */
    public static function checkNobodyDisp($type, $rec)
    {
        // ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
        global $loginUserType;
        // **************************************************************************************
     ...
     */
    public static function getParamForEntry($type, $id)
    {
        $db = GMList::getDB($type);
    ...
    {
        // フリーワードが指定されている場合
        if (isset($param['free']) && strlen($param['free'])) {
    ...
    public static function isHandle($type)
    {
        // ** conf.php で定義した定数の中で、利用したい定数をココに列挙する。 *******************
        global $loginUserType;
        // **************************************************************************************
	 ...
	 * @param type $rec
	 */
	public static function AdjustDayEndTime($rec){
