...
class Search
{
	...
	function setParamertorSet(&$args){
		...
		for($i=0; $i<count($db->colName); $i++)
		{
			...
			if( ( !isset( $args[ $column_name . '_PAL' ] ) || !is_array( $args[ $column_name . '_PAL' ] ) || !count( $args[ $column_name . '_PAL' ] ) ) ) //PAL未指定の場合
			{
				...
				else if( isset( $this->gm->colType[ $column_name ] ) && 'timestamp' == $this->gm->colType[ $column_name ] ) //検索カラムがタイムスタンプの場合
				{
	...
	function getFormStringSetVariable( $html, $jump = null, $partkey = null )
	{
		...
		$this->gm->addForm	 .= $this->addHiddenForm;
		$this->addHiddenForm = "";
		$this->gm->setVariable('form_begin','<form name="search_form" method="get" action="'. $jump .'" style="margin: 0px 0px;">'.$this->gm->addForm , false);
		$this->gm->setVariable('form_end','</form>' , false);
		$this->gm->addForm = "";
		$this->gm->authenticity_token = false;
		$buffer	 = $this->gm->getFormString($html, null, null, $partkey, 'buffer');
	...
	function getResult( $args = null )
	{
		...
		foreach( $this->param as $column_name => $param ){
			...
			if(is_array($this->value[ $column_name ])){
				$buf_array=array();
				foreach( $this->value[ $column_name ] as $key => $val){
					if( !empty($val) || $val === "0" ){
						$buf_array[ $key ] = $val;
					}
				}
				$this->value[ $column_name ] = $buf_array;
	...
	function searchTable( $db , $table , $name , $param , $data , $join_db = null ){
		...
		switch( $param[0] )
		{
			case 'alias'://古い記法  現在は非推奨。  新しいもに関してはsearchAliasを参照
				...
				$ttable = $tdb->getTable();
				if( $param[1] == 'comp' ){
					$ttable = $tdb->searchTable( $ttable, $param[3] , "==" , $data );
				}else{
					$ttable = $tdb->searchTable( $ttable, $param[3] , "=" , '%'.$data.'%' );
				}
			...
			case 'match':
				...
				switch( $param[1] )
				{
					...
					case 'empty':
						...
						break;
					case 'num_fuzzy':
						$table = $this->searchExecute( $db, $join_db, $table, $name, 'num_fuzzy', ($data));
						break;
				}
			break;
			case 'unmatch':
				...
				switch( $param[1] )
				{
					...
					case 'keyword':
						// キーワード検索の場合
						$table = $this->searchNotKeyword( $db, $table, $name, $data, array($name), $join_db );
						break;
	...
	function searchOR( &$db , $table , $name, $data, $join_db = null , $param = null , $unmatch = false ){
		...
		for($k=0; $k<count($data); $k++)	{
			...
			{
				$colType = ( $join_db ? $join_db->colType[ $name ] : $db->colType[ $name ] );
	...
	function searchBetween( &$db , $table , $name, $dataA, $dataB , $join_db = null , $unmatch = false ){
		...
		$colType = $this->gm->colType;
		if($join_db != NULL){
			if(isset($join_db->colType[$name])){	// alias の時
				$colType = $join_db->colType;
			}
		}
	...
	function searchMultipleTable( &$db , $table , $name , $data , $param, $type , $join_db = null ){
		...
		for($k=0;$k<$cnt;$k++){
			...
			for($l=0; $l<$trow; $l++){
				$ttable = $tdb->getTable();
				$ttable	 = $tdb->searchTable( $ttable, $param[$key_num], '=', $atable[$l] );
	 ...
	 * @return 検索結果のテーブル
	 */
	function searchNotKeyword( &$db , $table , $name , $data , $param , $join_db = null , $unmatch = false ){
		if(   is_array(  $data  )   )
		{
			// 値が配列の場合はエラー
			throw new InternalErrorException('Search param error -> '. $name. '_PAL[] is unmatch+keyword. but '. $name .' is array.');
		}
		$data	 = str_replace( "　", " ", ($data) );
		$key	 = explode(  ' ', $data  );
		$ttable	 = null;
	...
	function joinTimestamp( $column_name, &$args ){
		...
		if( !empty($args[$y_a]) || !empty($args[$m_a]) || !empty($args[$d_a]) ){
			$_month = 1;
			if(isset($args[$m_a])){
				$_month = strlen($args[$m_a]) ? $args[$m_a]-0 : 1;
			}
			$_day = 1;
			if(isset($args[$d_a])){
				$_day = strlen($args[$d_a]) ? $args[$d_a]-0 : 1;
			}
			$_year = 1970;
			if(isset($args[$y_a])){
				$_year = strlen($args[$y_a]) ? $args[$y_a]-0 : 1970;
			}
		...
		if( !empty($args[$y_b]) || !empty($args[$m_b]) || !empty($args[$d_b]) ){
			$_month = 12;
			if(isset($args[$m_b])){
				$_month = strlen($args[$m_b]) ? $args[$m_b]-0 : 12;
			}
			$_day = 31;
			if(isset($args[$d_b])){
				$_day = strlen($args[$d_b]) ? $args[$d_b]-0 : 31;
			}
			$_year = 2069;
			if(isset($args[$y_b])){
				$_year = strlen($args[$y_b]) ? $args[$y_b]-0 : 2069;
			}
			$time = mktime(0,0,-1, $_month, $_day, $_year);
