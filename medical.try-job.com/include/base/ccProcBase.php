<?php
	...
	class ccProcBase //
	{
		...
		static function valueReplace(&$gm, $rec, $cc)
		{
			$ret = "";
		...
		static function valueValueReplace(&$gm, $rec, $cc)
		{
			$ret = "";
        ...
        static function arrayReplace(&$gm, $rec, $cc)
        {
			$ret = "";
		...
		static function alias(&$_gm, $rec, $cc)
		{
			...
			for( $i=0 ; $i<$cnt && $value != '' ; $i++ )
			{
				...
				$db = $gm->getDB();
				if( isset(self::$alias_cache[$tables[$i]]) && isset(self::$alias_cache[$tables[$i]][$vals[$i]]) && isset(self::$alias_cache[$tables[$i]][$vals[$i]][$value]) ){
					$rec = self::$alias_cache[$tables[$i]][$vals[$i]][$value];
				}else{
					...
					self::$alias_cache[$tables[$i]][$vals[$i]][$value] = $rec;
				}
		...
		static function valueAlias(&$_gm, $_rec, $cc)
		{
			$ret = "";
			...
			if( isset(self::$alias_cache[$cc[1]]) && isset(self::$alias_cache[$cc[1]][ $cc[3]]) && isset(self::$alias_cache[$cc[1]][ $cc[3]][$cc[2]]) ){
				$rec = self::$alias_cache[$cc[1]][ $cc[3]][$cc[2]];
			}else{
				$table = $db->getTable($_gm->table_type);
				...
				$rec   = $db->getRecord( $table, 0 );
				self::$alias_cache[$cc[1]][ $cc[3]][$cc[2]] = $rec;
			}
		...
		static function arrayAlias(&$_gm, $_rec, $cc)
		{
			$ret = array();
			...
			foreach( $array as $key ){
				...
				if( isset(self::$alias_cache[$cc[1]]) && isset(self::$alias_cache[$cc[1]][$cc[3]]) && isset(self::$alias_cache[$cc[1]][$cc[3]][$key]) ){
					$arec = self::$alias_cache[$cc[1]][$cc[3]][$key];
				}else{
					$stable	 = $db->searchTable( $table, $cc[3], '=', $key );
					if( $db->getRow( $stable ) == 0 ) { continue; }
					$arec	 = $db->getRecord( $stable, 0 );
					self::$alias_cache[$cc[1]][$cc[3]][$key] = $arec;
				}
				...
				//戻り値が空の場合のデフォルトをセット
				$result = "";
				if( isset($cc[6]) ) { $result = $cc[6]; }
				return $result;
			}
		}

		// /で結合した置換後の文字列配列を返す。 
		static function arrayValueAlias(&$_gm, $_rec, $cc)
		{
			$ret = array();
	...
	class CommandBase extends command_base //
	{
        ...
        function groupTableSelectForm( &$gm , $rec , $args ){
            ...
            for($i=0;$i<$prow;$i++){
				...
				$ctable = $cdb->getTable();
                $ctable = $cdb->searchTable( $ctable, $args[6] , '=' , $pdb->getData( $prec , 'id' ) );
        ...
        function searchGroupTableForm( &$gm , $rec , $args ){
            ...
            for($i=0;$i<$prow;$i++){
				...
				$ctable = $cdb->getTable();
                $ctable = $cdb->searchTable( $ctable, $args[6] , '=' , $pid );
                $crow = $cdb->getRow( $ctable );
		...
		*/
		function listing( &$iGM , $iRec , $iArgs ) //
		{
			...
			if( !empty($query[ 'row' ]) ) //最大行数の指定がある場合
				{ $table = $db->limitOffset( $table , 0 , $query[ 'row' ] ); }

			$row = $db->getRow( $table );
			
			if( empty($query[ 'sort' ]) ) //ソートが指定されていない場合
			{
			...
			if( !isset($_SESSION[ 'search_query_index' ]) ) //クエリキャッシュのインデックスがない場合
			...
			if( !$row ) //検索結果が空の場合
			{
				$this->addBuffer( $gm->getCCResult( $iRec , '<!--# adapt ' . $query[ 'listingID' ] . '_failed #-->' ) );
				return;
			}

			$repeat = ( !empty($query[ 'row' ]) ? $query[ 'row' ] : $row );
			$table->onCache();

			$this->addBuffer( $gm->getCCResult( $iRec , '<!--# adapt ' . $query[ 'listingID' ] . '_head #-->' ) );

			if( !empty($query[ 'row' ]) && $originRow > $query[ 'row' ] ) //表示数より検索結果が多い場合
				{ $this->addBuffer( $gm->getCCResult( $iRec , '<!--# adapt ' . $query[ 'listingID' ] . '_head_over #-->' ) ); }

			
			$random = isset($query[ 'random' ]);
			if( $random )
			{
				 $numList = range(0, $row-1);
				 shuffle($numList);
			}
			...
			for( $i = 0 ; $repeat > $i ; ++$i ) //全ての行を処理
			{
				...
				{
					if( $random )
					{ $rec = $db->getRecord( $table , $numList[$i] ); }
					else
					{ $rec = $db->getRecord( $table , $i ); }

					$gm->setVariable( 'num' , $i + 1 );

					if( !empty($query[ 'source_nobr' ]) ) //ソースの改行無効指定がある場合
