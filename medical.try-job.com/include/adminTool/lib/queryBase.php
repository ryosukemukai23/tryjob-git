	...
	class QueryBase //
	{
		...
		*/
		static function CreateTable( $iTableName , $iColumns , $iIndexes = Array() ) //
		{
			...
			if( strlen($indexes)>0 ) //インデックスがある場合
		...
		static function UpdateRecord( $iTableName , $iColumns , $iRow , $iKeyColumn = 'id' ) //
		{
			...
			foreach( $iColumns as $column => $option ) //全てのカラムを処理
			{
				if( 'fake' == $option[ 'type' ] ) //擬似カラムの場合
					...
					if(isset($iRow[ $column ])){
						$row_column = $iRow[ $column ];
					}else{
						$row_column = null;
					}
					$params[] = QueryWriter::EscapeUpdateQueryValue( $row_column , $option[ 'type' ] ); 
			...
			if ($iKeyColumn) {
				$row_column = null;
				if (isset($iRow[$iKeyColumn])) {
					$row_column = $iRow[$iKeyColumn];
				}
				$params[] = QueryWriter::EscapeUpdateQueryValue($row_column, $iColumns[$iKeyColumn]['type']);
			}
