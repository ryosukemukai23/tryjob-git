	...
	*/
	class InstallStatus //
	{
		...
		static function Skipable() //
		{
			...
			{
				if( !empty($_SESSION[ 'firstToolAccess' ]) && is_file( self::$InstallConfPath ) ) //初回アクセスではない場合
		...
		*/
		static function Get( $iStatusName ) //
		{
			...
			if( !isset( $SYSTEM_INSTALL_STATUS ) )
				{ return null; }

			return isset($SYSTEM_INSTALL_STATUS[ $iStatusName ]) ? $SYSTEM_INSTALL_STATUS[ $iStatusName ] : null;
		}
		...
		*/
		static function Set( $iStatusName , $iState ) //
		{
			...
			fputs( $fp , '<?php' . "\n" );
			SystemUtil::safe_chmod( self::$InstallConfPath,0666 );
