<?php
	...
	*/
		...
		*/
		function ShutdownHandler()
		{
			if( $this->shutdownErrorLog )
			{
				...
				if( !is_null( $errStr ) ){
					if(substr($this->workDirectory,-1)!='/'){
						$path = $this->workDirectory.'/';
					}else{
						$path = $this->workDirectory;
					}
					$path .= $this->errorLogFile;
					$this->OutputErrorLog( $errStr , $path );
				}
			}
		}
		...
		function GetExceptionStr( $e_ )
		{
			...
			foreach( $array as $trace )
			{
				if( array_key_exists( 'file' , $trace ) ){
					...
					if( !isset($trace[ 'args' ][2]) ) { $trace[ 'args' ][2] = ""; }
					if( !isset($trace[ 'args' ][3]) ) { $trace[ 'args' ][3] = ""; }
		...
		*/
		function OutputErrorLog( $str_ , $filePath_ = null )
		{
			...
			if( $fp )
			{
				...
				if( $this->maxlogFileSize < filesize( $path ) ) //ログファイルの最大サイズを超えている場合
				{
					...
					fclose( $fp );
					SystemUtil::safe_chmod( $path, 0666 );
				}
			}
		}
	}
	...
	//ハンドラ登録
	function ErrorManager_ErrorHandler( $errNo_ , $errStr_ , $errFile_ , $errLine_ )
	{
		$object = new ErrorManager();
		$object->ErrorHandler( $errNo_ , $errStr_ , $errFile_ , $errLine_ );
	}
