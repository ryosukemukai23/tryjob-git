	...
	*/
	class AppLoginModel extends AppBaseModel //
	{
		...
		*/
		function doLogin() //
		{
			...
			if( isset( $_POST[ $LOGIN_KEY_FORM_NAME ] ) && isset( $_POST[ $LOGIN_PASSWD_FORM_NAME ] ) ) //ログイン情報が送信されている場合
			{
				...
				{
					if( !isset($THIS_TABLE_IS_USERDATA[ $target ]) || !$THIS_TABLE_IS_USERDATA[ $target ] ) //ユーザーテーブルではない場合
						{ continue; }

					if(!empty($_POST[ $LOGIN_KEY_FORM_NAME ]) && !empty($_POST[ $LOGIN_PASSWD_FORM_NAME ]))
			...
			if( $success ) //ログインに成功した場合
			{
				...
				if( isset( $_POST[ 'auto_login' ] ) && ( 'TRUE' == $_POST[ 'auto_login' ] ) ) //自動ログインの指定がある場合
					{ AutoLoginLogic::SaveLoginInfo( $loginType , $loginID ); }
			}
			else //ログインに失敗した場合
		...
		*/
		...
		{
			if( !empty($_POST[ 'redirect_id' ]) && !empty($_SESSION[ 'redirect_path' ][ $_POST[ 'redirect_id' ] ]) ) //リダイレクト先の指定がある場合
				{ SystemUtil::innerLocation( $_SESSION[ 'redirect_path' ][ $_POST[ 'redirect_id' ] ] ); }
			else if( !empty($_POST[ 'redirect_path' ]) ) //リダイレクト先の指定がある場合
				{ $this->loginResultURL = $_POST[ 'redirect_path' ]; }
			else if( !empty($_SESSION[ 'previous_page' ]) ) //直前のページがある場合
			{
