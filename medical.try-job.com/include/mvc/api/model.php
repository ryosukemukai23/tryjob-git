<?php
	...
	*/
	class AppAPIModel extends AppBaseModel //
	{
		...
		{
			global $USE_API_TOKEN_CHECK;
			global $loginUserType;
			global $NOT_LOGIN_USER_TYPE;
			global $USER_API_TOKEN_NAME;

			if( !$USE_API_TOKEN_CHECK )
				{ return true; }

			if( isset($this->param[ 'user_api_token' ]) && $this->param[ 'user_api_token' ] == $_SESSION[ $USER_API_TOKEN_NAME ] )
				{ return true; }

			throw new InvalidArgumentException( 'APIトークンが不正です( ' . http_build_query( $this->param ) . ' )' );
		}
		/**
			@brief APIを呼び出す。
		*/
