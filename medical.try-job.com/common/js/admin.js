...
function deleteSelectUser( type, id )
{
	...
	if(flag)
	{
		jQuery.ajax({
			...
			type: 'POST',
			dataType: "text",
			data: modifyApiParam("c=UserApi&m=deleteSelectUser&type=" + type + "&idList=" + id)
		})
...
function deleteActivateNoneUser( type )
{
	...
	if(flag)
	{
		jQuery.ajax({
			...
			type: 'POST',
			dataType: "text",
			data: modifyApiParam("c=UserApi&m=deleteActivateNone&type=" + type)
		})
		.done(function(res){ window.location.reload(true); })
		.fail(function(xml, status, e){});
	}
	...
	if(charges){
		...
		if(flag)
		{
			jQuery.ajax({
				...
				type : 'POST',
				dataType : "text",
				data : modifyApiParam("c=JobApi&m=setCharges&charges="+charges)
			})
	...
	if( year.length > 0 && month.length > 0 && day.length > 0 )
	{
		...
		if(flag)
		{
			jQuery.ajax({
				...
				type: 'POST',
				dataType: "text",
				data: modifyApiParam("c=UserApi&m=setLimitAll&type=" + type + "&year=" + year + "&month=" + month + "&day=" + day)
			})
			.done(function(res) {
				switch (res) {
...
function setcUserCharges( charges )
{
	...
	if(flag)
	{
		jQuery.ajax({
			...
			type: 'POST',
			dataType: "text",
			data: modifyApiParam("c=UserApi&m=setCharges&charges=" + charges)
		})
		.done(function(res){
			switch(res){
...
function editAddsDisp()
{
	...
	if(flag)
	{
		jQuery.ajax({
			...
			type : 'POST',
			dataType : "text",
			data : modifyApiParam("c=AreaApi&m=editAddsDisp&idList="+idList)
		})
...
function addSubSortCategory( tableName, id, sort_pal, adds_id )
{
	jQuery.ajax({
		...
		type: 'POST',
		dataType: "text",
		data: modifyApiParam("c=AreaApi&m=rankSort&tableName=" + tableName + "&id=" + id + "&sort_pal=" + sort_pal + "&adds_id=" + adds_id)
	})
	.done(function(res){ window.location.reload(true); })
	.fail(function(xml, status, e){ });
}
