...
$(function () {
  ...
  ha_init();
  $("select[name=has_add_sub]").on("change", function () {
    able2add_ha();
  });
  ...
  $("input[name=add_ha]").on("click", function () {
    ...
    hope_work_place.push(
      $("select[name=has_adds] option:selected").val() + "," + $("select[name=has_add_sub] option:selected").val()
    );
    var selectAddSubId = $("select[name=has_add_sub] option:selected").val();

    $("input[name=hope_work_place]").before(
      '<div><span id="addHAID_' +
        addHACount +
        '" class="ha_del" onclick="ha_del(this);">×削除</span>' +
        $("select[name=has_adds] option:selected").text() +
        " " +
        $("select[name=has_add_sub] option:selected").text() +
        "</div>"
    );
    $("#addHAID_" + addHACount)
      .parent()
      .addClass("addHopeList");
    ...
    $(this).after(
      '<span class="hint" style="color: red; margin-bottom: .8rem;">希望勤務地を追加しました。<br />さらに追加するには再び都道府県から選択して下さい</span>'
    );
    $("#drawhope_work_placeSelecter span.hint")
      .delay(3000)
