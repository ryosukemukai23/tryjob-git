...
//			$db->setData($rec,"activate",$ACTIVE_ACCEPT);
		...
		if($check){
			...
			if(isset($_POST["dotw_text"]) && preg_match("/sun/",$_POST["dotw_text"])){
			...
			if(isset($_POST["dotw_text"]) && preg_match("/mon/",$_POST["dotw_text"])){
			...
			if(isset($_POST["dotw_text"]) && preg_match("/tue/",$_POST["dotw_text"])){
			...
			if(isset($_POST["dotw_text"]) && preg_match("/wed/",$_POST["dotw_text"])){
			...
			if(isset($_POST["dotw_text"]) && preg_match("/thr/",$_POST["dotw_text"])){
			...
			if(isset($_POST["dotw_text"]) && preg_match("/fri/",$_POST["dotw_text"])){
			...
			if(isset($_POST["dotw_text"]) && preg_match("/sat/",$_POST["dotw_text"])){
	...
	function infoCheck( &$gm, &$rec, $loginUserType, $loginUserRank )
	{
		...
		default:
			$db	 = $gm[ $_GET['type'] ]->getDB();
			$check = ($db->getData($rec,"activate") == 4);
			break;
		}
		return $check;
	}
	...
	function editProc( &$gm, &$rec, $loginUserType, $loginUserRank ,$check = false)
	{
		...
		if(isset($_POST["dotw"])){	// 営業日チェックボックス
			$_POST["dotw_text"]=implode("/",$_POST["dotw"]);
		}
	...
	function restoreComp( &$gm, &$rec, &$old_rec, $loginUserType, $loginUserRank )
	{
		...
		$table = $rdb->getTable('d');
		$table = $rdb->searchTable( $table, 'owner', '=', $id );
