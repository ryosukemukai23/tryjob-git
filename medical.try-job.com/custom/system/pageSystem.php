<?php
	...
	class pageSystem extends System
	{
		...
		function registCheck( &$gm, $edit, $loginUserType, $loginUserRank , $check = false)
		{
			...
			if($result){
				...
				foreach( $authority as $auth ){
					$table_buf[] = $db->searchTable( $table, 'authority', 'in', '%'.$auth.'%' );
				}
				$table =  $db->orTableM($table_buf);
		...
		{
			$MODE = "";
			if(class_exists("mod_special") && isset($_GET["mode"]) )
			{
				if($_GET["mode"]=="special" || $_POST["mode"]=="special") { $MODE="_SP"; }
			}
