<?php
	...
	*/
	class AppmidDeleteController extends AppDeleteController //
	{
		//■処理 //
		...
		*/
		function __construct() //
		{
			...
			if ($loginUserType == "cUser") {
				...
				Concept::IsTrue(pay_jobLogic::isAvailable($LOGIN_ID, "mid"))->OrThrow("midContractExpire");
			}
			Concept::IsTrue(JobLogic::isHandle($_GET['type']))->OrThrow("IllegalAccess");

			if( isset($_REQUEST[ 'del_edit' ]) ) //入力フォームを使用する場合
			{
			...
			{
				if( isset( $_POST[ 'post' ] ) && 'delete' == $_POST[ 'post' ] ) //削除実行処理が要求されている場合
					{ $this->action = 'doDelete'; }
				else //要求がない場合
