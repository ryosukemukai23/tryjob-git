<?php
	...
	*/
	class AppgiftEditModel extends AppBaseModel //
	{
		//■処理 //
		...
		*/
		function doEdit() //
		{
			...
			{
				if( $column == "shadow_id" || $column == "delete_key" ) { continue; }
		...
		function updateQuery() //
		{
			...
			foreach( $this->db->colName as $column ) //全てのカラムを処理
			{
				...
				else //入力がない場合
				{
					...
					{
						if( !isset( $_POST[ $column . '_DELETE' ] ) || 'true' != $_POST[ $column . '_DELETE' ] ) //削除要求がない場合
							{ $useOriginData = true; }
					}
		...
		*/
		function __construct() //
		{
			...
			if( class_exists( 'mod_nobody' ) ){
				$hash = "";
				if( isset($_GET["md5"]) ) { $hash = $_GET["md5"]; }
				if(nobodyLogic::giftActivateCheck($_GET["id"],$hash)){
					$nobodyID = SystemUtil::getTableData("gift", $_GET["id"], "owner");
					nobodyLogic::setUserID($nobodyID);
				}
			}
