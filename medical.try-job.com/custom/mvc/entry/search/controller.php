<?php
	...
	*/
	class AppentrySearchController extends AppSearchController //
	{
		//■処理 //
		...
		*/
		function __construct() //
		{
			...
			if($loginUserType == "cUser"){
				Concept::IsTrue(pay_jobLogic::isAvailable($LOGIN_ID, "mid") || pay_jobLogic::isAvailable($LOGIN_ID, "fresh"))->OrThrow("entryContractExpire");
			}

			if( isset($_GET[ 'inquiry' ]) ) //問い合わせ要求がある場合
				{ $this->action = 'doInquiry'; }
			else //要求がない場合
			{
				if( isset($_GET[ 'run' ]) ) //検索要求がある場合
