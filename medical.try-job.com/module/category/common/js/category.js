...
function deleteCheckCategory(tableName, name, id) {
    ...
    if (flag) {
        jQuery.ajax({
			...
			type: 'POST',
			dataType: "text",
			data: modifyApiParam("c=CategoryApi&m=delete&tableName=" + tableName + "&id=" + id)
		})
...
function sortCategory( tableName, id, sort_pal )
{
	jQuery.ajax({
		...
		type: 'POST',
		dataType: "text",
		data: modifyApiParam("c=CategoryApi&m=rankSort&tableName=" + tableName + "&id=" + id + "&sort_pal=" + sort_pal)
	})
	.done(function(res){ window.location.reload(true); })
	.fail(function(xml, status, e){ });
