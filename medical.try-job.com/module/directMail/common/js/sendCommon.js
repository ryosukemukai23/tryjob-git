...
SendCommon.prototype = {
	...
	sendMail:function(){
		...
		jQuery.ajax({
			...
			type: 'POST',
			dataType: "json",
			data: modifyApiParam('c=mailSendApi&m=send&id=' + self.recID + '&receive_id=' + receive)
		})
		.done(function(res){
			if(res.faled){
				self.faled_cnt++;
	...
	sendComp:function(){
		...
		jQuery.ajax({
			...
			type: 'POST',
			dataType: "text",
			data: modifyApiParam('c=mailSendApi&m=complete&id=' + this.recID + '&success_id=' + this.success_id.join("/") + '&through_id=' + this.through_id.join("/") + '&faled_id=' + this.faled_id.join("/"))
		})
		.done(function(res){
	...
	drawList:function(){
		jQuery.ajax({
			...
			type: 'POST',
			dataType: "json",
			data: modifyApiParam('c=mailSendApi&m=drawSendList&id=' + this.recID + '&current=0')
		})
		.done(function(res){
	...
	abort:function(){
		...
		jQuery.ajax({
			...
			type: 'POST',
			dataType: "text",
			data: modifyApiParam('c=mailSendApi&m=reSendComp&id=' + this.recID + '&success_id=' + this.success_id.join("/") + '&through_id=' + this.through_id.join("/") + '&faled_id=' + this.faled_id.join("/"))
		}).done(function(res){
				$('#main_message').html( ' メールの送信が完了しました。<br><br>'+self.sendTotalCnt + '件中' + self.success_cnt +'件のメールの送信に成功、'+self.through_cnt+'件が送信除外されました。');
				self.drawList();
		}).fail(function(){
