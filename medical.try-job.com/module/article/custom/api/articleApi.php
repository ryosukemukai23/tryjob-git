...
class mod_articleApi
{
	...
	function edit( &$param )
	{
		...
		if( $check1 && $check2 )
		{
			...
			$articleTitle = $db->getData($rec, 'name'); // 修正：記事タイトルカラムは通常title
			$articleId = $db->getData($rec, 'id');       // 現在のレコードから取得

			// 公開処理（activateが4に変更）かつcUserによる処理の場合
			if ($loginUserType == 'cUser' && $old_activate != '4' && $new_activate == '4') {
				PointLogic::addPoints($owner, 100, "記事(ID: {$articleId})「{$articleTitle}」が公開されました。");
			}
			// ▲▲▲ DOTRAISE（修正版）▲▲▲
	...
	static function returnJsonData($res) {
