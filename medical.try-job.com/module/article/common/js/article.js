...
function articleEdit(type){
    ...
    switch(type) {
        case 'open': // admin用: 即公開
            $form.find('[name=activate]').val('4');
            break;
        case 'close': // 下書きへ戻す
            $form.find('[name=activate]').val('1');
            break;
        case 'request': // cUserが公開申請
            $form.find('[name=activate]').val('2');
            break;
        case 'approve': // adminが承認
            $form.find('[name=activate]').val('3');
            break;
        case 'publish': // cUserが承認済み記事を公開
            $form.find('[name=activate]').val('4');
            break;
    }
